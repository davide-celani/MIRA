...
# `mira_data_long()` also works when the script is sourced outside an R
# package.  The Stan model uses the numeric likelihood id, while users work
# with readable names.
.mira_prepare_outcome_key_long <- function(x) {
...
.mira_prepare_likelihood_long <- function(likelihood, outcome) {
...
.mira_sample_skewness_long <- function(x) {
  x <- as.numeric(x)
  n <- length(x)
  s <- stats::sd(x)
...
.mira_empty_covariate_map_long <- function() {
  data.frame(
    index = integer(0),
    name = character(0),
    label = character(0),
...
.mira_empty_covariate_variables_long <- function() {
  data.frame(
    name = character(0),
    label = character(0),
    type = character(0),
...
.mira_covariate_label_long <- function(x, fallback) {
...
.mira_normalize_reference_levels_long <- function(reference_levels) {
...
.mira_covariate_problem_long <- function(x) {
...
.mira_prepare_covariates_long <- function(
    data,
    covariates,
    excluded_names,
    reference_levels,
  ...
  S <- nrow(data)
  reference_levels <- .mira_normalize_reference_levels_long(reference_levels)
  ...
  add_excluded_long <- function(name, reason) {
    excluded <<- rbind(
      excluded,
  ...
  if (mode == "explicit") {
    ...
    for (name in candidates) {
      problem <- .mira_covariate_problem_long(data[[name]])
      if (is.null(problem)) {
        selected <- c(selected, name)
      } else {
        add_excluded_long(name, problem)
      }
    }
  } else {
    selected <- character(0)
  ...
  for (name in selected) {
    x <- data[[name]]
    problem <- .mira_covariate_problem_long(x)
    if (!is.null(problem)) {
      if (mode == "explicit") {
    ...
    original_label <- .mira_covariate_label_long(x, name)
  ...
  columns <- if (length(column_rows) > 0L) {
    ...
    .mira_empty_covariate_map_long()
  }
  variables <- if (length(variable_rows) > 0L) {
    do.call(rbind, variable_rows)
  } else {
    .mira_empty_covariate_variables_long()
  }
...
mira_data_long <- function(
  ...
  if (tolower(requested_outcome) == "auto") {
    ...
    requested_key <- .mira_prepare_outcome_key_long(requested_outcome)
    ...
    if (length(exact_match) == 1L) {
      ...
      alias_match <- available_outcomes[
        vapply(
          available_outcomes,
          function(x) identical(.mira_prepare_outcome_key_long(x), requested_key),
          logical(1L)
        )
      ]
  ...
  outcome_class <- .mira_prepare_outcome_key_long(outcome_name)
  if (tolower(requested_outcome) == "generic") {
    outcome_class <- "generic"
  } else if (tolower(requested_outcome) != "auto" &&
             .mira_prepare_outcome_key_long(requested_outcome) %in% c("BCVA", "CMT")) {
    outcome_class <- .mira_prepare_outcome_key_long(requested_outcome)
  }

  likelihood <- .mira_prepare_likelihood_long(likelihood, outcome_class)
  ...
  covariate_design <- .mira_prepare_covariates_long(
    data = data,
  ...
  outcome_diagnostics <- list(
    ...
    skewness = .mira_sample_skewness_long(y),
    log_skewness = if (all(y > 0)) .mira_sample_skewness_long(log(y)) else NA_real_,
  ...
  list(
    ...
    # R-side metadata; mira_fit_long() does not pass these to Stan
