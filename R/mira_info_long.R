...
.mira_key_long <- function(x) {
  tolower(gsub("[^[:alnum:]]+", "_", trimws(as.character(x))))
}

.mira_compact_unique_long <- function(x) {
  unique(x[!is.na(x) & nzchar(x)])
}

.mira_validate_name_long <- function(x, data, argument, allow_null = FALSE) {
...
.mira_column_profile_long <- function(data) {
  n <- nrow(data)
  data.frame(
    variable = names(data),
...
.mira_time_order_long <- function(token) {
  key <- .mira_key_long(token)
...
.mira_parse_longitudinal_name_long <- function(variable, variable_pattern = "auto") {
  if (is.function(variable_pattern)) {
    ...
    order <- if (is.null(parsed$time_order)) .mira_time_order_long(label) else {
  ...
  if (!identical(variable_pattern, "auto")) {
    ...
    return(list(
      ...
      time_order = .mira_time_order_long(pieces[[3L]]),
...
.mira_make_longitudinal_group_long <- function(data,
                                          variables,
                                          outcome = NULL,
  ...
  parsed <- lapply(variables, .mira_parse_longitudinal_name_long,
  ...
  if (is.null(outcome) || !nzchar(outcome)) {
    candidates <- .mira_compact_unique_long(parsed_outcomes)
...
.mira_detect_longitudinal_variables_long <- function(data, variable_pattern = "auto") {
  parsed_rows <- list()
  counter <- 1L

  for (position in seq_along(data)) {
    parsed <- .mira_parse_longitudinal_name_long(names(data)[[position]], variable_pattern)
    if (is.null(parsed)) next
    parsed_rows[[counter]] <- data.frame(
      outcome = parsed$outcome,
      outcome_key = .mira_key_long(parsed$outcome),
...
.mira_detect_id_long <- function(data, id = NULL, exclude = character(0)) {
  if (!is.null(id)) {
    selected <- .mira_validate_name_long(id, data, "id")
    return(list(
...
.mira_detect_arm_long <- function(data, arm = NULL, exclude = character(0)) {
  if (!is.null(arm)) {
    selected <- .mira_validate_name_long(arm, data, "arm")
    return(list(
      selected = selected, automatic = FALSE, source = "manual",
      candidates = data.frame(), alternatives = character(0), warnings = character(0)
    ))
...
.mira_classify_covariates_long <- function(data, variables) {
  numeric <- character(0)
...
.mira_detect_covariates_long <- function(data, covariates = NULL, exclude = character(0)) {
  reserved <- c(
  ...
  if (!is.null(covariates) && !(length(covariates) == 1L && identical(covariates, "auto"))) {
    ...
    classes <- .mira_classify_covariates_long(data, covariates)
    return(list(
  ...
  candidate <- candidate[!grepl("(^id$|_id$|^id_)", .mira_key_long(candidate))]
  classes <- .mira_classify_covariates_long(data, candidate)
  ...
  selected_classes <- .mira_classify_covariates_long(data, selected)
  list(
    selected = selected,
...
.mira_skipped_module_long <- function(package = NA_character_, reason) {
  list(
    performed = FALSE,
    package = package,
    object = NULL,
...
.mira_quote_formula_name_long <- function(x) {
  paste0("`", gsub("`", "", x, fixed = TRUE), "`")
}

.mira_formula_text_long <- function(x) {
  if (is.null(x)) return(NA_character_)
  paste(deparse(x, width.cutoff = 500L), collapse = " ")
}

.mira_make_fixed_formula_long <- function(arm_enabled, covariates = character(0)) {
  core <- if (isTRUE(arm_enabled)) "time_factor * arm_factor" else "time_factor"
  covariate_terms <- if (length(covariates) > 0L) {
    vapply(covariates, .mira_quote_formula_name_long, character(1L))
  } else character(0)
...
.mira_make_lmer_formula_long <- function(core, covariates, random_term) {
  covariate_terms <- if (length(covariates) > 0L) {
    vapply(covariates, .mira_quote_formula_name_long, character(1L))
  } else character(0)
...
.mira_as_numeric_long <- function(x) {
...
.mira_column_name_long <- function(x, candidates = character(0), contains = character(0)) {
...
.mira_adjust_family_long <- function(table, p_col = "p_raw", primary = "holm") {
  if (is.null(table)) return(NULL)
  table <- as.data.frame(table, stringsAsFactors = FALSE)
  if (!p_col %in% names(table)) {
    detected <- .mira_column_name_long(
      table,
      candidates = c("p.value", "p_value", "p", "Pr(>|t|)", "Pr(>|z|)",
                     "Pr(>|W|)", "Pr(>F)")
    )
    if (!is.na(detected)) {
      table$p_raw <- .mira_as_numeric_long(table[[detected]])
      p_col <- "p_raw"
    } else {
  ...
  p <- .mira_as_numeric_long(table[[p_col]])
  adjust_long <- function(method) {
  ...
  table$p_raw <- p
  table$p_primary <- adjust_long(primary)
  table$primary_method <- rep(primary, nrow(table))
  table$p_bonferroni <- adjust_long("bonferroni")
  table$p_holm <- adjust_long("holm")
  table$p_bh <- adjust_long("BH")
  table$p_by <- adjust_long("BY")
  table
}

.mira_emm_summary_long <- function(object, primary, adjust = "none", confidence_level = 0.95) {
  if (is.null(object)) return(NULL)
  tab <- as.data.frame(
    summary(object, infer = c(TRUE, TRUE), adjust = adjust, level = confidence_level),
    stringsAsFactors = FALSE
  )
  if (identical(adjust, "none")) .mira_adjust_family_long(tab, primary = primary) else tab
}

.mira_prepare_advanced_data_long <- function(long_data,
                                        time_levels,
                                        arm_enabled,
                                        arm_levels,
                                        covariates,
...
.mira_run_emmeans_long <- function(model, arm_enabled, primary_method, confidence_level) {
  if (is.null(model)) {
    return(.mira_skipped_module_long(
      "emmeans",
  ...
  warnings <- character(0)
  errors <- list()
  safe_long <- function(name, expr) {
    evaluated <- .mira_eval_long(expr)
    warnings <<- unique(c(warnings, evaluated$warnings))
    if (!is.null(evaluated$error)) errors[[name]] <<- evaluated$error
    evaluated$value
  }

  time_emm <- safe_long("time", emmeans::emmeans(model, ~ time_factor))
  if (is.null(time_emm)) {
    return(list(
  ...
  time_pairwise <- safe_long(
    "time_pairwise",
    emmeans::contrast(time_emm, method = "pairwise", adjust = "none")
  )
  time_pairwise_tidy <- safe_long(
    "time_pairwise_summary",
    .mira_emm_summary_long(time_pairwise, primary_method, confidence_level = confidence_level)
  )
  time_pairwise_tukey <- safe_long(
    "time_pairwise_tukey",
    .mira_emm_summary_long(
  ...
  if (!is.null(trends)) {
    ...
    if (!is.null(trend_grid) && "contrast" %in% names(trend_grid)) {
      ...
      subsetted_trends <- safe_long("ordinal_polynomial_trend_subset", trends[keep])
      if (!is.null(subsetted_trends)) trends <- subsetted_trends
    }
  }
  trends_tidy <- safe_long(
    "ordinal_polynomial_trends_summary",
    .mira_emm_summary_long(trends, primary_method, confidence_level = confidence_level)
  )
  ...
  if (isTRUE(arm_enabled)) {
    arm_emm <- safe_long("arm", emmeans::emmeans(model, ~ arm_factor))
    arm_tidy <- safe_long(
      "arm_summary",
      if (!is.null(arm_emm)) {
        as.data.frame(
    ...
    arm_time_emm <- safe_long(
      "arm_time",
      emmeans::emmeans(model, ~ arm_factor * time_factor)
    )
    arm_time_tidy <- safe_long(
      "arm_time_summary",
    ...
    arm_by_time <- safe_long(
      "arm_by_time",
      emmeans::emmeans(model, ~ arm_factor | time_factor)
    )
    simple_arm <- safe_long(
      "simple_arm",
      if (!is.null(arm_by_time)) {
        emmeans::contrast(arm_by_time, method = "pairwise", adjust = "none")
      } else NULL
    )
    simple_arm_tidy <- safe_long(
      "simple_arm_summary",
      .mira_emm_summary_long(simple_arm, primary_method, confidence_level = confidence_level)
    )

    time_by_arm <- safe_long(
      "time_by_arm",
      emmeans::emmeans(model, ~ time_factor | arm_factor)
    )
    simple_time <- safe_long(
      "simple_time",
      if (!is.null(time_by_arm)) {
        emmeans::contrast(time_by_arm, method = "pairwise", adjust = "none")
      } else NULL
    )
    simple_time_tidy <- safe_long(
      "simple_time_summary",
      .mira_emm_summary_long(simple_time, primary_method, confidence_level = confidence_level)
    )

    interaction <- safe_long(
    ...
    interaction_tidy <- safe_long(
      "interaction_contrasts_summary",
      .mira_emm_summary_long(interaction, primary_method, confidence_level = confidence_level)
    )
  }
...
.mira_standardize_afex_long <- function(table) {
  if (is.null(table)) return(NULL)
  out <- as.data.frame(table, stringsAsFactors = FALSE)
  effect_col <- .mira_column_name_long(out, candidates = c("Effect", "term"))
  ...
  stat_col <- .mira_column_name_long(out, candidates = c("F", "F.value", "F value"))
  p_col <- .mira_column_name_long(out, candidates = c("Pr(>F)", "p", "p.value"))
  num_df_col <- .mira_column_name_long(out, candidates = c("num Df", "num_df", "Df"))
  den_df_col <- .mira_column_name_long(out, candidates = c("den Df", "den_df"))
  pes_col <- .mira_column_name_long(out, candidates = c("pes", "partial eta squared"), contains = "pes")
  ges_col <- .mira_column_name_long(out, candidates = c("ges", "generalized eta squared"), contains = "ges")
  value_long <- function(column) {
    if (is.na(column)) rep(NA_real_, nrow(out)) else .mira_as_numeric_long(out[[column]])
  }
  data.frame(
    effect = effect,
    statistic = value_long(stat_col),
    df_num = value_long(num_df_col),
    df_den = value_long(den_df_col),
    p_value = value_long(p_col),
    partial_eta_squared = value_long(pes_col),
    generalized_eta_squared = value_long(ges_col),
    stringsAsFactors = FALSE
  )
}

.mira_run_rm_anova_long <- function(data,
                               arm_enabled,
                               covariates,
                               categorical_covariates) {
  ...
  object <- fitted$value
  raw_pes_eval <- .mira_eval_long(stats::anova(object, correction = "none", es = "pes"))
  raw_ges_eval <- .mira_eval_long(stats::anova(object, correction = "none", es = "ges"))
  gg_eval <- .mira_eval_long(stats::anova(object, correction = "GG", es = "pes"))
  hf_eval <- .mira_eval_long(stats::anova(object, correction = "HF", es = "pes"))
  summary_eval <- .mira_eval_long(summary(object))

  raw <- .mira_standardize_afex_long(raw_pes_eval$value)
  ges <- .mira_standardize_afex_long(raw_ges_eval$value)
  gg <- .mira_standardize_afex_long(gg_eval$value)
  hf <- .mira_standardize_afex_long(hf_eval$value)
...
.mira_rank_biserial_paired_long <- function(delta) {
...
.mira_run_friedman_long <- function(analysis_data, time_vars, time_labels, primary_method) {
  complete <- analysis_data[
    stats::complete.cases(analysis_data[time_vars]),
    time_vars,
    drop = FALSE
  ]
  global_eval <- .mira_eval_long(stats::friedman.test(as.matrix(complete)))
  ...
  for (i in seq_len(length(time_vars) - 1L)) {
    for (j in seq.int(i + 1L, length(time_vars))) {
      ...
      rows[[counter]] <- data.frame(
        ...
        rank_biserial = .mira_rank_biserial_paired_long(y - x),
        stringsAsFactors = FALSE
      )
      counter <- counter + 1L
    }
  ...
  posthoc <- .mira_adjust_family_long(posthoc, p_col = "p_raw", primary = primary_method)
...
.mira_effect_indices_long <- function(coefficient_names, question) {
...
.mira_wald_chisq_long <- function(beta, covariance, indices) {
...
.mira_gee_coefficients_long <- function(model, confidence_level) {
  ...
  estimate_col <- .mira_column_name_long(table, candidates = "Estimate")
  se_col <- .mira_column_name_long(
    table,
    candidates = c("Std.err", "Std. Error", "San.se"),
    contains = c("stderr", "sanse")
  )
  estimate <- if (!is.na(estimate_col)) .mira_as_numeric_long(table[[estimate_col]]) else
    rep(NA_real_, nrow(table))
  robust_se <- if (!is.na(se_col)) .mira_as_numeric_long(table[[se_col]]) else
    rep(NA_real_, nrow(table))
...
.mira_run_gee_long <- function(data, fixed_formula, confidence_level) {
  if (!requireNamespace("geepack", quietly = TRUE)) {
    skipped <- .mira_skipped_module_long(
      "geepack",
  ...
  for (name in names(structures)) {
    ...
    model <- fitted$value
    summary_eval <- .mira_eval_long(summary(model))
    coefficient_eval <- .mira_eval_long(.mira_gee_coefficients_long(model, confidence_level))
    qic_eval <- .mira_eval_long(geepack::QIC(model))
    anova_eval <- .mira_eval_long(stats::anova(model))
    beta_eval <- .mira_eval_long(stats::coef(model))
    vcov_eval <- .mira_eval_long(stats::vcov(model))
    effect_rows <- if (!is.null(beta_eval$value) && !is.null(vcov_eval$value)) {
      lapply(c("TIME", "ARM", "TIME_X_ARM"), function(question) {
        test <- .mira_wald_chisq_long(
          beta_eval$value,
          vcov_eval$value,
          .mira_effect_indices_long(names(beta_eval$value), question)
        )
...
.mira_lmer_diagnostics_long <- function(model) {
  if (is.null(model)) {
...
.mira_run_random_slope_long <- function(data,
                                   arm_enabled,
                                   covariates,
  ...
  formula <- .mira_make_lmer_formula_long(full_core, covariates, random_term)
  fitted <- .mira_eval_long(
  ...
  model <- fitted$value
  diagnostics <- .mira_lmer_diagnostics_long(model)
  summary_eval <- .mira_eval_long(summary(model))
  anova_eval <- .mira_eval_long(stats::anova(model))
  fixed_eval <- .mira_eval_long(as.data.frame(coef(summary(model)), stringsAsFactors = FALSE))
  ...
  comparison_eval <- .mira_eval_long(
  ...
  fit_reduced_long <- function(name, core) {
    if (is.null(ml_eval$value)) return(NULL)
    reduced_formula <- .mira_make_lmer_formula_long(core, covariates, random_term)
    reduced_eval <- .mira_eval_long(
      lme4::lmer(
    ...
    test_eval <- .mira_eval_long(stats::anova(reduced_eval$value, ml_eval$value))
  ...
  if (isTRUE(arm_enabled)) {
    interaction <- fit_reduced_long("interaction", "time_factor + arm_factor")
    global_time <- fit_reduced_long("global_time", "arm_factor")
    global_arm <- fit_reduced_long("global_arm", "time_factor")
  } else {
    global_time <- fit_reduced_long("global_time", "1")
  }
...
.mira_run_nlme_long <- function(data, fixed_formula) {
...
.mira_run_robust_inference_long <- function(model, confidence_level) {
  if (is.null(model)) {
    return(.mira_skipped_module_long(
      "clubSandwich",
  ...
  covariance_eval <- .mira_eval_long(clubSandwich::vcovCR(model, type = "CR2"))
  ...
  for (question in c("TIME", "ARM", "TIME_X_ARM")) {
    indices <- .mira_effect_indices_long(names(coefficients), question)
...
.mira_run_r2_long <- function(model) {
  if (is.null(model)) {
    return(.mira_skipped_module_long(
      "performance",
  ...
  evaluated <- .mira_eval_long(performance::r2_nakagawa(model))
...
.mira_qic_value_long <- function(qic, name) {
  ...
  if (is.matrix(qic) || is.data.frame(qic)) {
    ...
    if (column > 0L) return(.mira_as_numeric_long(qic[1L, column])[[1L]])
  }
  values <- unlist(qic, use.names = TRUE)
  index <- match(tolower(name), tolower(names(values)), nomatch = 0L)
  if (index > 0L) .mira_as_numeric_long(values[[index]])[[1L]] else NA_real_
}

.mira_build_model_comparison_long <- function(data,
                                         fixed_formula,
                                         mixed_model,
                                         mixed_converged,
                                         mixed_singular,
  ...
  likelihood_long <- function(model, fun) {
    if (is.null(model)) return(NA_real_)
    tryCatch(as.numeric(fun(model)), error = function(e) NA_real_)
  }
  row_long <- function(model_name,
                  model,
                  formula,
                  converged,
                  singular,
    ...
    data.frame(
      model = model_name,
      formula = .mira_formula_text_long(formula),
      n = safe_nobs_long(model),
      n_subjects = default_subjects,
      AIC = if (likelihood_based) likelihood_long(model, stats::AIC) else NA_real_,
      BIC = if (likelihood_based) likelihood_long(model, stats::BIC) else NA_real_,
      logLik = if (likelihood_based) likelihood_long(model, stats::logLik) else NA_real_,
      QIC = qic,
      CIC = cic,
  ...
  ri_formula <- if (!is.null(mixed_model)) {
    ...
    stats::as.formula(paste(
      .mira_formula_text_long(fixed_formula),
      "+ (1 | patient_factor)"
    ))
  }
  ...
  for (name in c("independence", "exchangeable", "ar1")) {
    ...
    rows[[length(rows) + 1L]] <- row_long(
      ...
      qic = .mira_qic_value_long(item$qic, "QIC"),
      cic = .mira_qic_value_long(item$qic, "CIC")
    )
  }
...
.mira_sensitivity_row_long <- function(question,
                                  method,
                                  statistic = NA_real_,
                                  df = NA_character_,
                                  p_raw = NA_real_,
...
.mira_lrt_sensitivity_long <- function(test, question, method, object_path, n, note = NA_character_) {
  if (is.null(test)) return(NULL)
  table <- tryCatch(as.data.frame(test), error = function(e) NULL)
  if (is.null(table) || nrow(table) == 0L) return(NULL)
  p_col <- .mira_column_name_long(
    table,
    candidates = c("Pr(>Chisq)", "p.value", "p"),
    contains = c("prchisq", "pvalue")
  )
  statistic_col <- .mira_column_name_long(
    table,
    candidates = c("Chisq", "L.Ratio", "F", "Deviance"),
    contains = c("chisq", "lratio")
  )
  df_col <- .mira_column_name_long(
    table,
    candidates = c("Chi Df", "Df", "numDF"),
    contains = c("chidf", "numdf")
  )
  p <- if (!is.na(p_col)) .mira_as_numeric_long(table[[p_col]]) else rep(NA_real_, nrow(table))
  candidate <- which(is.finite(p))
  index <- if (length(candidate) > 0L) tail(candidate, 1L) else nrow(table)
  .mira_sensitivity_row_long(
    question = question,
    method = method,
    statistic = if (!is.na(statistic_col)) .mira_as_numeric_long(table[[statistic_col]])[[index]] else NA_real_,
    df = if (!is.na(df_col)) .mira_as_numeric_long(table[[df_col]])[[index]] else NA_real_,
    p_raw = p[[index]],
    n = n,
    object_path = object_path,
    note = note
...
.mira_match_effect_long <- function(table, question) {
...
.mira_build_sensitivity_long <- function(mixed_tests,
                                    rm_anova,
                                    friedman,
                                    gee,
                                    random_slope,
                                    ...
                                    n_default) {
  rows <- list()
  add_long <- function(value) {
    if (!is.null(value)) rows[[length(rows) + 1L]] <<- value
  }

  add_long(.mira_lrt_sensitivity_long(
  ...
  if (isTRUE(robust$performed) && length(robust$effect_tests) > 0L) {
    for (question in names(robust$effect_tests)) {
      ...
      f_col <- .mira_column_name_long(table, candidates = c("Fstat", "F"))
      dfn_col <- .mira_column_name_long(table, candidates = c("df_num", "df num"))
      dfd_col <- .mira_column_name_long(table, candidates = c("df_denom", "df denom"))
      p_col <- .mira_column_name_long(table, candidates = c("p_val", "p.value", "p"))
      add_long(.mira_sensitivity_row_long(
        question, "Mixed model: CR2/HTZ robust Wald test",
        if (!is.na(f_col)) .mira_as_numeric_long(table[[f_col]])[[1L]] else NA_real_,
        paste(
          if (!is.na(dfn_col)) .mira_as_numeric_long(table[[dfn_col]])[[1L]] else NA_real_,
          if (!is.na(dfd_col)) .mira_as_numeric_long(table[[dfd_col]])[[1L]] else NA_real_,
          sep = "/"
        ),
        if (!is.na(p_col)) .mira_as_numeric_long(table[[p_col]])[[1L]] else NA_real_,
...
.mira_multiplicity_family_long <- function(table, primary_method, object_path) {
  list(
...
.mira_run_advanced_inference_long <- function(analysis_data,
                                         long_data,
                                         time_vars,
                                         time_labels,
                                         arm_enabled,
  ...
  advanced_data <- .mira_prepare_advanced_data_long(
    long_data = long_data,
  ...
  fixed_formula <- .mira_make_fixed_formula_long(arm_enabled, covariates)
  friedman <- .mira_run_friedman_long(
    analysis_data, time_vars, time_labels, primary_method
  )

  if (isTRUE(model_enabled)) {
    emmeans <- .mira_run_emmeans_long(
      mixed_model, arm_enabled, primary_method, confidence_level
    )
    rm_anova <- .mira_run_rm_anova_long(
      advanced_data, arm_enabled, covariates, categorical_covariates
    )
    gee <- .mira_run_gee_long(advanced_data, fixed_formula, confidence_level)
    random_slope <- .mira_run_random_slope_long(
      advanced_data, arm_enabled, covariates, mixed_model
    )
    nlme_models <- .mira_run_nlme_long(advanced_data, fixed_formula)
    robust <- .mira_run_robust_inference_long(mixed_model, confidence_level)
  } else {
    reason <- "Advanced model-based analyses were disabled by model = FALSE."
    emmeans <- .mira_skipped_module_long("emmeans", reason)
    rm_anova <- .mira_skipped_module_long("afex", reason)
    random_slope <- .mira_skipped_module_long("lme4", reason)
    robust <- .mira_skipped_module_long("clubSandwich", reason)
    gee_item <- .mira_skipped_module_long("geepack", reason)
    gee <- list(
    ...
    nlme_item <- .mira_skipped_module_long("nlme", reason)
    nlme_models <- list(
  ...
  random_intercept_r2 <- .mira_run_r2_long(mixed_model)
  random_slope_r2 <- .mira_run_r2_long(random_slope$model)
  model_comparison <- .mira_build_model_comparison_long(
    advanced_data, fixed_formula, mixed_model, mixed_converged, mixed_singular,
    random_slope, nlme_models, gee
  )
  sensitivity <- .mira_build_sensitivity_long(
    mixed_tests = list(
...
.mira_failed_advanced_long <- function(error, warnings = character(0)) {
  reason <- "Advanced longitudinal inference was unavailable because its protected wrapper failed."
  skipped <- .mira_skipped_module_long(NA_character_, reason)
  skipped$error <- error
...
.mira_analyse_single_outcome_long <- function(data,
                                         id = "patient",
                                         time_vars = NULL,
                                         time_labels = NULL,
                                         arm = NULL,
  ...
  scalar_flag_long <- function(x, name) {
  ...
  scalar_flag_long(plots, "plots")
  scalar_flag_long(model, "model")
  scalar_flag_long(outliers, "outliers")
  scalar_flag_long(correlations, "correlations")
  scalar_flag_long(verbose, "verbose")
  scalar_flag_long(strict_id, "strict_id")
  scalar_flag_long(arm_tests, "arm_tests")
    ...
    stop(
      sprintf(
        paste0(
          ...
          "mira_info_long() assumes one row per subject in wide format. ",
  ...
  if (is.null(time_vars)) {
    detected <- .mira_detect_longitudinal_variables_long(data, "auto")$groups
    if (length(detected) != 1L) {
      stop(
        paste0(
          "The internal analyzer requires exactly one outcome. Use mira_info_long() ",
    ...
    parsed <- lapply(time_vars, .mira_parse_longitudinal_name_long, variable_pattern = "auto")
    ...
    if (!is.null(.outcome_name)) {
      ...
      inferred <- .mira_compact_unique_long(parsed_names)
  ...
  safe_mean_long <- function(x) {
    x <- x[is.finite(x)]
    if (length(x) == 0L) NA_real_ else mean(x)
  }

  safe_sd_long <- function(x) {
    x <- x[is.finite(x)]
    if (length(x) < 2L) NA_real_ else stats::sd(x)
  }

  safe_var_long <- function(x) {
    x <- x[is.finite(x)]
    if (length(x) < 2L) NA_real_ else stats::var(x)
  }

  safe_median_long <- function(x) {
    x <- x[is.finite(x)]
    if (length(x) == 0L) NA_real_ else stats::median(x)
  }

  safe_quantile_long <- function(x, p) {
    x <- x[is.finite(x)]
    if (length(x) == 0L) {
      NA_real_
    } else {
  ...
  safe_wilcox_p_long <- function(x, y) {
  ...
  safe_welch_test_long <- function(x, y) {
    x <- x[is.finite(x)]
    y <- y[is.finite(y)]
  ...
  safe_unpaired_wilcox_p_long <- function(x, y) {
    x <- x[is.finite(x)]
    y <- y[is.finite(y)]
  ...
  safe_welch_anova_p_long <- function(value, group) {
  ...
  safe_kruskal_p_long <- function(value, group) {
    keep <- is.finite(value) & !is.na(group)
    value <- value[keep]
    group <- droplevels(factor(group[keep]))
  ...
  improvement_counts_long <- function(delta) {
  ...
  descriptive_list <- lapply(time_vars, function(v) {
    ...
    ci <- mean_ci_long(x)
    q1 <- safe_quantile_long(x, 0.25)
    q3 <- safe_quantile_long(x, 0.75)
    m <- safe_mean_long(x)
    s <- safe_sd_long(x)
    ...
    data.frame(
      ...
      mean = m,
      sd = s,
      variance = safe_var_long(x),
      se = unname(ci["se"]),
      ci_lower = unname(ci["lower"]),
      ci_upper = unname(ci["upper"]),
      median = safe_median_long(x),
      q1 = q1,
      q3 = q3,
  ...
  for (i in seq_len(length(time_vars) - 1L)) {
    for (j in seq.int(i + 1L, length(time_vars))) {
      ...
      counts <- improvement_counts_long(delta)
      ...
      change_list[[counter]] <- data.frame(
        ...
        increased_pct = pct_long(counts$increased_n, n),
        decreased_n = counts$decreased_n,
        decreased_pct = pct_long(counts$decreased_n, n),
        stable_n = counts$stable_n,
        stable_pct = pct_long(counts$stable_n, n),
        improved_n = counts$improved_n,
        improved_pct = pct_long(counts$improved_n, n),
        worsened_n = counts$worsened_n,
        worsened_pct = pct_long(counts$worsened_n, n),
        paired_t_p = safe_t_p_long(x_complete, y_complete),
        wilcoxon_p = safe_wilcox_p_long(x_complete, y_complete),
        stringsAsFactors = FALSE
      )
  ...
  if (length(change_list) > 0L) {
    ...
    change$paired_t_p_adj <- adjust_p_long(change$paired_t_p)
    change$wilcoxon_p_adj <- adjust_p_long(change$wilcoxon_p)
  } else {
    change <- data.frame(
  ...
  if (arm_available && arm_tests && length(arm_levels) >= 2L) {
    ...
    arm_missingness$p_value_adj <- adjust_p_long(arm_missingness$p_value)
    ...
    omnibus_list <- lapply(seq_along(time_vars), function(k) {
      ...
      data.frame(
        ...
        welch_anova_p = safe_welch_anova_p_long(value, group),
        kruskal_p = safe_kruskal_p_long(value, group),
        stringsAsFactors = FALSE
      )
    })

    arm_time_omnibus <- do.call(rbind, omnibus_list)
    arm_time_omnibus$welch_anova_p_adj <- adjust_p_long(arm_time_omnibus$welch_anova_p)
    arm_time_omnibus$kruskal_p_adj <- adjust_p_long(arm_time_omnibus$kruskal_p)
    ...
    for (k in seq_along(time_vars)) {
      ...
      for (pair in arm_pairs) {
        ...
        pair_list[[p_counter]] <- data.frame(
          ...
          n_a = length(xa),
          n_b = length(xb),
          mean_a = safe_mean_long(xa),
          mean_b = safe_mean_long(xb),
          mean_difference_b_minus_a = wt$estimate,
          ci_lower = wt$conf.int[[1L]],
          ci_upper = wt$conf.int[[2L]],
          hedges_g = hedges_g_long(xa, xb),
          welch_t_p = wt$p.value,
          wilcoxon_p = safe_unpaired_wilcox_p_long(xa, xb),
          stringsAsFactors = FALSE
        )
    ...
    arm_time_pairwise$welch_t_p_adj <- adjust_p_long(arm_time_pairwise$welch_t_p)
    arm_time_pairwise$wilcoxon_p_adj <- adjust_p_long(arm_time_pairwise$wilcoxon_p)
    ...
    for (k in seq.int(2L, length(time_vars))) {
      ...
      for (g in arm_levels) {
        ...
        change_desc_list[[cd_counter]] <- data.frame(
          ...
          time_index = k,
          n = n,
          mean_change = safe_mean_long(delta),
          sd_change = safe_sd_long(delta),
          se_change = unname(ci["se"]),
          ci_lower = unname(ci["lower"]),
          ci_upper = unname(ci["upper"]),
          median_change = safe_median_long(delta),
          improved_n = counts$improved_n,
          improved_pct = pct_long(counts$improved_n, n),
          worsened_n = counts$worsened_n,
          worsened_pct = pct_long(counts$worsened_n, n),
      ...
      change_omnibus_list[[k - 1L]] <- data.frame(
        ...
        welch_anova_p = safe_welch_anova_p_long(delta_all, group),
        kruskal_p = safe_kruskal_p_long(delta_all, group),
        stringsAsFactors = FALSE
      )
      ...
      for (pair in arm_pairs) {
        ...
        change_pair_list[[cp_counter]] <- data.frame(
          ...
          n_a = length(da),
          n_b = length(db),
          mean_change_a = safe_mean_long(da),
          mean_change_b = safe_mean_long(db),
          difference_in_change_b_minus_a = wt$estimate,
          ci_lower = wt$conf.int[[1L]],
          ci_upper = wt$conf.int[[2L]],
          hedges_g = hedges_g_long(da, db),
          welch_t_p = wt$p.value,
          wilcoxon_p = safe_unpaired_wilcox_p_long(da, db),
          stringsAsFactors = FALSE
        )
    ...
    arm_change_omnibus$welch_anova_p_adj <- adjust_p_long(arm_change_omnibus$welch_anova_p)
    arm_change_omnibus$kruskal_p_adj <- adjust_p_long(arm_change_omnibus$kruskal_p)

    arm_change_pairwise <- do.call(rbind, change_pair_list)
    rownames(arm_change_pairwise) <- NULL
    arm_change_pairwise$welch_t_p_adj <- adjust_p_long(arm_change_pairwise$welch_t_p)
    arm_change_pairwise$wilcoxon_p_adj <- adjust_p_long(arm_change_pairwise$wilcoxon_p)
  }
  ...
  between_sd <- safe_sd_long(individual_means)
  grand_mean <- safe_mean_long(long_data$value)

  patient_means <- tapply(long_data$value, long_data$patient, safe_mean_long)
  ...
  within_sd <- safe_sd_long(residuals_within)
  ...
  if (model) {
    if (!requireNamespace("lme4", quietly = TRUE)) {
      ...
      if (nrow(model_data) < 3L || nlevels(model_data$patient_factor) < 2L ||
        ...
        fit_with_warnings_long <- function(expr) {
          withCallingHandlers(
            expr,
            warning = function(w) {
        ...
        quote_formula_name_long <- function(x) {
          paste0("`", gsub("`", "", x, fixed = TRUE), "`")
        }
        covariate_terms <- vapply(
          model_covariates_used,
          quote_formula_name_long,
          character(1L)
        )
        make_model_formula_long <- function(core) {
          stats::as.formula(paste(
            "value ~",
        ...
        model_formula <- if (model_uses_arm && nlevels(model_data$arm_factor) >= 2L) {
          make_model_formula_long("time_factor * arm_factor")
        } else {
          make_model_formula_long("time_factor")
        }

        mixed_model <- tryCatch(
          fit_with_warnings_long(
            if (requireNamespace("lmerTest", quietly = TRUE)) {
              lmerTest::lmer(
                model_formula,
                data = model_data,
        ...
        if (!is.null(mixed_model)) {
          ...
          if (model_uses_arm &&
            ...
            no_time_ml <- tryCatch(
              fit_with_warnings_long(
                lme4::lmer(
                  make_model_formula_long("arm_factor"),
                  data = model_data,
                  REML = FALSE,
                  na.action = stats::na.omit
                )
            ...
            global_time_test <- tryCatch({
              full_ml <- fit_with_warnings_long(
                lme4::lmer(
                  make_model_formula_long("time_factor"),
                  data = model_data,
                  REML = FALSE,
              ...
              null_ml <- fit_with_warnings_long(
                lme4::lmer(
                  make_model_formula_long("1"),
                  data = model_data,
                  REML = FALSE,
                  na.action = stats::na.omit
                )
  ...
  advanced_eval <- .mira_eval_long(
    .mira_run_advanced_inference_long(
  ...
  advanced_results <- if (is.null(advanced_eval$value)) {
    .mira_failed_advanced_long(advanced_eval$error, advanced_eval$warnings)
  } else {
    advanced_eval$value
  }
  ...
  class(result) <- c("mira_info_long", "list")
...
.mira_name_groups_long <- function(groups) {
...
.mira_match_group_long <- function(request, groups) {
  if (length(groups) == 0L) return(NA_integer_)
  hit <- which(.mira_key_long(names(groups)) == .mira_key_long(request))
  if (length(hit) == 1L) hit else NA_integer_
}

.mira_groups_from_time_vars_long <- function(data,
                                        time_vars,
                                        outcomes = NULL,
                                        variable_pattern = "auto") {
  groups <- list()
  ...
  if (is.list(time_vars) && !is.data.frame(time_vars)) {
    ...
    for (i in seq_along(time_vars)) {
      ...
      groups[[i]] <- .mira_make_longitudinal_group_long(
        data = data,
    ...
    return(.mira_name_groups_long(groups))
  }
  ...
  parsed <- lapply(time_vars, .mira_parse_longitudinal_name_long,
                   variable_pattern = variable_pattern)
  parsed_outcomes <- vapply(parsed, function(x) {
    if (is.null(x)) NA_character_ else x$outcome
  }, character(1L))
  parsed_keys <- .mira_compact_unique_long(.mira_key_long(parsed_outcomes))

  if (all(!is.na(parsed_outcomes)) && length(parsed_keys) > 1L) {
    for (key in parsed_keys) {
      idx <- which(.mira_key_long(parsed_outcomes) == key)
      groups[[length(groups) + 1L]] <- .mira_make_longitudinal_group_long(
        data = data,
    ...
    groups[[1L]] <- .mira_make_longitudinal_group_long(
      data = data,
      variables = time_vars,
      outcome = hint,
      variable_pattern = variable_pattern,
  ...
  .mira_name_groups_long(groups)
}

.mira_apply_time_labels_long <- function(groups, time_labels) {
  ...
  if (is.list(time_labels) && !is.data.frame(time_labels)) {
    ...
    for (i in seq_along(groups)) {
      labels <- if (length(groups) == 1L && (is.null(label_names) || !nzchar(label_names[[1L]]))) {
        ...
        idx <- which(.mira_key_long(label_names) == .mira_key_long(names(groups)[[i]]))
        if (length(idx) != 1L) {
...
.mira_resolve_outcome_groups_long <- function(data,
                                         detected,
                                         outcomes = NULL,
                                         time_vars = NULL,
                                         time_labels = NULL,
  ...
  if (!outcomes_auto && (!is.character(outcomes) || anyNA(outcomes) ||
                         any(!nzchar(outcomes)) || anyDuplicated(.mira_key_long(outcomes)))) {
  ...
  if (!is.null(time_vars)) {
    groups <- .mira_groups_from_time_vars_long(
      data, time_vars, outcomes = outcomes, variable_pattern = variable_pattern
    )
  } else {
    groups <- .mira_name_groups_long(detected$groups)
  ...
  if (!outcomes_auto) {
    ...
    if (!is.null(time_vars) && length(groups) == 1L && length(outcomes) == 1L) {
      ...
      for (requested in outcomes) {
        idx <- .mira_match_group_long(requested, groups)
        if (is.na(idx)) {
          ambiguous_idx <- .mira_match_group_long(requested, detected$ambiguous_groups)
  ...
  groups <- .mira_apply_time_labels_long(groups, time_labels)
...
.mira_resolve_direction_long <- function(value, outcome) {
  ...
  if (!is.null(names(value)) && any(nzchar(names(value)))) {
    ...
    idx <- which(.mira_key_long(names(value)) == .mira_key_long(outcome))
    if (length(idx) != 1L) {
...
.mira_resolve_threshold_long <- function(value, outcome) {
...
.mira_choose_reference_arm_long <- function(data, arm, reference_arm = NULL) {
  if (is.null(arm)) {
    if (!is.null(reference_arm)) {
...
.mira_build_analysis_config_long <- function(data,
                                        id = NULL,
                                        outcomes = NULL,
                                        time_vars = NULL,
                                        time_labels = NULL,
  ...
  detected_longitudinal <- .mira_detect_longitudinal_variables_long(data, variable_pattern)
  resolved <- .mira_resolve_outcome_groups_long(
    data = data,
  ...
  id_detection <- .mira_detect_id_long(data, id = id, exclude = all_long_vars)
  analysis_data <- data
  id_generated <- FALSE
  ...
  arm_detection <- .mira_detect_arm_long(
    data,
    arm = arm,
    exclude = unique(c(all_long_vars, selected_id))
  )
  selected_arm <- arm_detection$selected
  reference <- .mira_choose_reference_arm_long(data, selected_arm, reference_arm)

  covariate_detection <- .mira_detect_covariates_long(
    data,
    covariates = covariates,
    exclude = unique(c(
  ...
  directions <- lapply(names(groups), function(outcome) {
    .mira_resolve_direction_long(improvement_direction, outcome)
  })
  names(directions) <- names(groups)
  thresholds <- lapply(names(groups), function(outcome) {
    .mira_resolve_threshold_long(stable_threshold, outcome)
  })
  ...
  data_overview <- list(
    n_rows = nrow(data),
    n_columns = ncol(data),
    column_profile = .mira_column_profile_long(data)
  )
...
#' and detection rules as [mira_info_long()]. No outcome-specific statistical
#' analysis is performed.
#'
#' `mira_detect_long()` provides the configuration-only workflow directly. The same
#' result can be requested from `mira_info_long()` with `inspect_only = TRUE`, which
#' additionally resolves the optional-analysis switches accepted by that
#' function.
#'
#' @inheritParams mira_info_long
#'
#' @return Invisibly returns an object with classes `mira_detect_long` and `list`.
...
#'
#' @seealso [mira_info_long()]
#'
#' @export
mira_detect_long <- function(data,
                        id = NULL,
                        outcomes = NULL,
                        time_vars = NULL,
                        time_labels = NULL,
  ...
  built <- .mira_build_analysis_config_long(
  ...
  class(result) <- c("mira_detect_long", "list")
  if (verbose) print(result)
  invisible(result)
}
...
#'   invisibly. Assigning the call, for example `fit <- mira_info_long(data)`, still
...
#'   returns an empty `mira_info_multi_long` container.
...
#'   configuration warnings, and return a `mira_detect_long` object without running
#'   any outcome analysis. The resolved `analyses` settings are included in
#'   `config`. `verbose = TRUE` prints the detection report.
#'
#' @details
#' `mira_info_long()` first validates global arguments and resolves the analysis
...
#' `detected_variables`.
#'
#' Consequently, `mira_info_long(data)` selects every unambiguous detected outcome,
...
#' available, failures become `mira_info_error_long` entries, and one warning names
...
#' error is fatal for a single outcome but is isolated as a `mira_info_error_long`
#' entry during a multi-outcome call.
#'
#' @return Invisibly returns an object whose class and structure depend on the
#'   requested mode.
#'
#' With `inspect_only = TRUE`, the result has classes `mira_detect_long` and `list`
#' and contains:
#'
#' \describe{
...
#' }
#'
#' A successful single-outcome analysis has classes `mira_info_long` and `list`.
...
#' }
#'
#' A multi-outcome result has classes `mira_info_multi_long`, `mira_info_long`, and `list`.
...
#' Each successful element of `outcomes` is an outcome-specific `mira_info_long`
#' object with the components above. A failed element has classes
#' `mira_info_error_long` and `list` and contains `outcome` and `error`. Use
...
#'
#' @seealso [mira_detect_long()] for configuration inspection without analysis,
...
#'
#' # ------------------------------------------------------------------
#' # Example 2: Audit what mira_info_long() detected before fitting anything
#' # ------------------------------------------------------------------
...
#'     emm$time$tidy
#'
#'     compact_contrasts_long <- function(table) {
#'       if (is.null(table)) return(NULL)
#'       keep <- intersect(
...
#'     compact_contrasts_long(emm$time$pairwise_tidy)
#'     compact_contrasts_long(emm$baseline_final$tidy)
#'
...
#'     compact_contrasts_long(emm$arm_time$simple_arm_tidy)
#'
...
#'     compact_contrasts_long(emm$trends$tidy)
#'   }
#'
#'   # ----------------------------------------------------------------
...
#' # not values estimated or clinically validated by mira_info_long().
...
#' # complete single-outcome mira_info_long object with the same internal paths.
...
#' baseline_to_final_long <- function(result) {
#'   result$change[
...
#' )
#' lapply(multi_fit$outcomes, baseline_to_final_long)
#'
...
#'   verbose = FALSE
#' )
#' response_counts_long <- function(result) {
#'   lapply(
#'     result$outcomes,
...
#' }
#' list(
#'   zero_threshold = response_counts_long(multi_zero),
#'   practical_threshold = response_counts_long(multi_fit)
#' )
#'
#' \donttest{
#' # The multi-outcome plot method needs outcome = when several outcomes succeed.
#' if (requireNamespace("ggplot2", quietly = TRUE)) {
#'   multi_plot_fit <- mira_info_long(
#'     multi_data,
#'     outcomes = c("symptom", "function"),
#'     improvement_direction = directions,
#'     stable_threshold = practical_thresholds,
...
#' missing_fit <- mira_info_long(
#'   missing_data,
#'   analyses = "correlations",
#'   verbose = FALSE
#' )
#'
#' # mira_info_long() does not impute. Descriptives use all finite observations at
...
#' }
#' rbind(
#'   none = module_switches_long(cfg_none),
#'   arm_tests = module_switches_long(cfg_arm),
#'   model_correlations = module_switches_long(cfg_model_cor),
#'   all = module_switches_long(cfg_all)
#' )
#'
...
mira_info_long <- function(data,
  ...
  for (flag_name in names(flags)) {
    scalar_flag_long(flags[[flag_name]], flag_name)
  }
  if (!is.numeric(alpha) || length(alpha) != 1L || !is.finite(alpha) ||
      alpha <= 0 || alpha >= 1) {
    stop("alpha must be a number strictly between 0 and 1.", call. = FALSE)
  ...
  for (outcome_name in names(built$groups)) {
    ...
    analyse_long <- function() {
      .mira_analyse_single_outcome_long(
        data = built$data,
        id = built$config$id,
    ...
    if (!inherits(outcome_result, "mira_info_error_long")) {
  ...
  if (length(outcome_results) == 1L && length(failed) == 0L) {
    ...
    class(result) <- c("mira_info_multi_long", "mira_info_long", "list")
  }
...
.mira_print_output_guide_long <- function(x, title = "AVAILABLE OUTPUT ELEMENTS") {
  cat("\n", title, "\n", sep = "")
  cat(strrep("-", 84L), "\n", sep = "")
...
#' @export
print.mira_detect_long <- function(x, ...) {
  cat("MIRA INFO\n")
  cat(strrep("=", 84L), "\n", sep = "")
  cat("\nDETECTION OVERVIEW\n")
  cat(strrep("-", 84L), "\n", sep = "")
  ...
  .mira_print_output_guide_long(x)
  invisible(x)
}

#' @export
print.mira_info_multi_long <- function(x, ...) {
  cat("MIRA INFO\n")
  cat(strrep("=", 84L), "\n", sep = "")
  ...
  rows <- lapply(names(x$outcomes), function(outcome) {
    ...
    if (inherits(result, "mira_info_error_long")) {
...
#' @export
print.mira_info_error_long <- function(x, ...) {
  cat("MIRA INFO\n")
  cat(strrep("=", 84L), "\n", sep = "")
  cat("\nANALYSIS ERROR\n")
  cat(strrep("-", 84L), "\n", sep = "")
...
#' @export
print.mira_info_long <- function(x,
                            digits = 3,
                            max_rows = 20,
                            correlations = TRUE,
                            model = TRUE,
  ...
  line_long <- function(char = "-", n = 84L) cat(strrep(char, n), "\n", sep = "")
  ...
  extract_test_p_long <- function(test) {
  ...
  if (!is.null(d) && nrow(d) > 0L) {
    ...
    cat(sprintf(
      ...
      fmt_num_long(baseline_row$mean), fmt_num_long(baseline_row$sd)
    ))
    if (nrow(d) > 1L) {
      ...
      cat(sprintf(
        ...
        fmt_num_long(final_row$mean), fmt_num_long(final_row$sd)
      ))
    }
  }
  ...
  if (!is.null(x$missing$by_time) && nrow(x$missing$by_time) > 0L) {
    ...
    cat(sprintf(
      ...
      fmt_pct_long(if (total_cells > 0L) available_cells / total_cells * 100 else NA_real_)
    ))
  }
  ...
  if (!is.null(baseline_final) && nrow(baseline_final) == 1L) {
    ...
    cat(sprintf(
      ...
      fmt_num_long(bf_key$mean_change), conf_label,
      fmt_num_long(bf_key$ci_lower), fmt_num_long(bf_key$ci_upper),
      fmt_num_long(bf_key$cohens_dz), effect_label_long(bf_key$cohens_dz)
    ))
    cat(sprintf(
      "Adjusted paired tests: t=%s | Wilcoxon=%s\n",
      fmt_p_long(bf_key$paired_t_p_adj), fmt_p_long(bf_key$wilcoxon_p_adj)
    ))
    if (x$settings$improvement_direction != "unknown") {
      cat(sprintf(
        "Clinical direction: improved=%s | stable=%s | worsened=%s\n",
        fmt_pct_long(bf_key$improved_pct), fmt_pct_long(bf_key$stable_pct),
        fmt_pct_long(bf_key$worsened_pct)
      ))
    }
  }
  ...
  if (model) {
    ...
    if (!is.null(x$model$global_time_test)) {
      global_tests <- c(
        global_tests,
        paste0("time p=", fmt_p_long(extract_test_p_long(x$model$global_time_test)))
      )
    }
    if (!is.null(x$model$global_arm_test)) {
      global_tests <- c(
        global_tests,
        paste0("arm p=", fmt_p_long(extract_test_p_long(x$model$global_arm_test)))
      )
    }
    if (!is.null(x$model$arm_time_interaction_test)) {
      global_tests <- c(
        ...
        paste0(
          "time-by-arm p=",
          fmt_p_long(extract_test_p_long(x$model$arm_time_interaction_test))
        )
      )
    }
  ...
  if (!is.null(x$config$auto_detected)) {
    ...
    mode_long <- function(value) if (isTRUE(value)) "automatic" else "manual"
    id_mode <- if (isTRUE(x$config$id_generated)) "generated" else mode_long(automatic$id)
    arm_mode <- if (is.null(x$config$arm)) "none" else mode_long(automatic$arm)
    cat(sprintf(
      "Selection mode: ID=%s | outcome=%s | time variables=%s | arm=%s | covariates=%s\n",
      id_mode, mode_long(automatic$outcomes), mode_long(automatic$time_vars),
      arm_mode, mode_long(automatic$covariates)
    ))
  ...
  # DESCRIPTIVES
  # ------------------------------------------------------------------
  section_long("DESCRIPTIVE STATISTICS")
  ...
  if (isTRUE(ov$arm_analysis) && !is.null(x$arm_analysis)) {
    ...
    section_long("TREATMENT ARM EXPLORATION")
    ...
    if (!is.null(x$arm_analysis$descriptives) &&
      ...
      cat("\nDescriptives by arm and time:\n")
      print(limit_table_long(ad_print, "time-by-arm rows"), row.names = FALSE)
    }
    ...
    if (!is.null(x$arm_analysis$missingness) &&
      ...
      am_print <- data.frame(
        Time = am$time_label,
        Test = am$test,
        p = vapply(am$p_value, fmt_p_long, character(1L)),
        p_adj = vapply(am$p_value_adj, fmt_p_long, character(1L)),
        check.names = FALSE
      )
    ...
    if (!is.null(x$arm_analysis$baseline_balance) &&
      ...
      bb_print <- data.frame(
        ...
        Welch_p = vapply(bb$welch_t_p, fmt_p_long, character(1L)),
        Wilcoxon_p = vapply(bb$wilcoxon_p, fmt_p_long, character(1L)),
        check.names = FALSE
      )

      cat("\nBaseline balance (exploratory; arm B - arm A):\n")
      print(limit_table_long(bb_print, "baseline comparisons"), row.names = FALSE)
    }
    ...
    if (!is.null(x$arm_analysis$time_omnibus) &&
      ...
      ao_print <- data.frame(
        Time = ao$time_label,
        N = ao$n,
        Welch_ANOVA_p = vapply(ao$welch_anova_p, fmt_p_long, character(1L)),
        Welch_ANOVA_p_adj = vapply(ao$welch_anova_p_adj, fmt_p_long, character(1L)),
        Kruskal_p = vapply(ao$kruskal_p, fmt_p_long, character(1L)),
        Kruskal_p_adj = vapply(ao$kruskal_p_adj, fmt_p_long, character(1L)),
        check.names = FALSE
      )
    ...
    if (!is.null(x$arm_analysis$time_pairwise) &&
      ...
      ap_print <- data.frame(
        ...
        Welch_p = vapply(ap$welch_t_p, fmt_p_long, character(1L)),
        Welch_p_adj = vapply(ap$welch_t_p_adj, fmt_p_long, character(1L)),
        Wilcoxon_p = vapply(ap$wilcoxon_p, fmt_p_long, character(1L)),
        Wilcoxon_p_adj = vapply(ap$wilcoxon_p_adj, fmt_p_long, character(1L)),
        check.names = FALSE
      )

      cat("\nPairwise arm comparisons by timepoint:\n")
      print(limit_table_long(ap_print, "arm pairwise comparisons"), row.names = FALSE)
    }
    ...
    if (!is.null(x$arm_analysis$change_descriptives) &&
      ...
      print(limit_table_long(acd_print, "arm change descriptive rows"), row.names = FALSE)
    }
    ...
    if (!is.null(x$arm_analysis$change_omnibus) &&
      ...
      co_print <- data.frame(
        ...
        N = co$n,
        Welch_ANOVA_p = vapply(co$welch_anova_p, fmt_p_long, character(1L)),
        Welch_ANOVA_p_adj = vapply(co$welch_anova_p_adj, fmt_p_long, character(1L)),
        Kruskal_p = vapply(co$kruskal_p, fmt_p_long, character(1L)),
        Kruskal_p_adj = vapply(co$kruskal_p_adj, fmt_p_long, character(1L)),
        check.names = FALSE
      )
    ...
    if (!is.null(x$arm_analysis$change_pairwise) &&
      ...
      cp_print <- data.frame(
        ...
        Welch_p = vapply(cp$welch_t_p, fmt_p_long, character(1L)),
        Welch_p_adj = vapply(cp$welch_t_p_adj, fmt_p_long, character(1L)),
        Wilcoxon_p = vapply(cp$wilcoxon_p, fmt_p_long, character(1L)),
        Wilcoxon_p_adj = vapply(cp$wilcoxon_p_adj, fmt_p_long, character(1L)),
        check.names = FALSE
      )

      cat("\nBetween-arm comparison of baseline-to-follow-up change:\n")
      print(limit_table_long(cp_print, "change comparisons"), row.names = FALSE)
    }
  ...
  if (missingness && !is.null(x$missing)) {
    section_long("MISSINGNESS / DATA AVAILABILITY")
    ...
    if (!is.null(x$missing$by_patient) && nrow(x$missing$by_patient) > 0L) {
      ...
      cat(sprintf("\nSubjects with >=1 unavailable longitudinal value: %d/%d (%s)\n",
                  ...
                  fmt_pct_long(mean(mbp$unavailable_n > 0L) * 100)))
      cat(sprintf("Total unavailable cells: %d of %d (%s)\n",
                  sum(mbp$unavailable_n), nrow(mbp) * ov$n_timepoints,
                  fmt_pct_long(sum(mbp$unavailable_n) / (nrow(mbp) * ov$n_timepoints) * 100)))

      if (nrow(affected) == 0L) {
        cat("No subjects have missing or non-finite longitudinal values.\n")
      } else {
        cat("\nAffected subjects:\n")
        affected <- limit_table_long(affected, "affected subjects")
  ...
  # LONGITUDINAL CHANGE
  # ------------------------------------------------------------------
  section_long("LONGITUDINAL CHANGE")
  ...
  if (is.null(x$change) || nrow(x$change) == 0L) {
    ...
    if (nrow(baseline_final) == 1L) {
      ...
      cat(sprintf("Mean: %s -> %s | Mean change: %s | SD change: %s | SE change: %s\n",
                  fmt_num_long(bf$mean_from), fmt_num_long(bf$mean_to), fmt_num_long(bf$mean_change),
                  fmt_num_long(bf$sd_change), fmt_num_long(bf$se_change)))
      cat(sprintf("%s: [%s, %s] | Cohen dz: %s (%s)\n",
                  conf_label, fmt_num_long(bf$ci_lower), fmt_num_long(bf$ci_upper),
                  fmt_num_long(bf$cohens_dz), effect_label_long(bf$cohens_dz)))
      cat(sprintf("Increase / decrease / stable: %s / %s / %s\n",
                  fmt_pct_long(bf$increased_pct), fmt_pct_long(bf$decreased_pct), fmt_pct_long(bf$stable_pct)))

      if (x$settings$improvement_direction != "unknown") {
        cat(sprintf("Improved / worsened / stable: %s / %s / %s\n",
                    fmt_pct_long(bf$improved_pct), fmt_pct_long(bf$worsened_pct), fmt_pct_long(bf$stable_pct)))
      } else {
      ...
      cat(sprintf("Paired t p: %s (adjusted %s) | Wilcoxon p: %s (adjusted %s)\n",
                  fmt_p_long(bf$paired_t_p), fmt_p_long(bf$paired_t_p_adj),
                  fmt_p_long(bf$wilcoxon_p), fmt_p_long(bf$wilcoxon_p_adj)))
    }
    ...
    pairwise_print <- data.frame(
      ...
      t_p = vapply(ch$paired_t_p, fmt_p_long, character(1L)),
      t_p_adj = vapply(ch$paired_t_p_adj, fmt_p_long, character(1L)),
      Wilcoxon_p = vapply(ch$wilcoxon_p, fmt_p_long, character(1L)),
      Wilcoxon_p_adj = vapply(ch$wilcoxon_p_adj, fmt_p_long, character(1L)),
      check.names = FALSE
    )
    ...
    pairwise_print <- limit_table_long(pairwise_print, "pairwise comparisons")
  ...
  # VARIABILITY / ICC
  # ------------------------------------------------------------------
  section_long("WITHIN- / BETWEEN-SUBJECT VARIABILITY")
  v <- x$variability[1L, ]
  ...
  if (all(c("ms_between", "ms_within", "df_between", "df_within") %in% names(v))) {
    cat(sprintf("ANOVA components - MS between: %s | MS within: %s | df: %s / %s\n",
                fmt_num_long(v$ms_between), fmt_num_long(v$ms_within),
                fmt_num_long(v$df_between, 0L), fmt_num_long(v$df_within, 0L)))
  }
  ...
  # ------------------------------------------------------------------
  if (model) {
    section_long("MIXED-EFFECTS MODEL")
    ...
    if (!is.null(x$model$error)) {
      ...
      cat(sprintf("Covariates requested: %s\n",
                  fmt_text_long(x$model$covariates_requested)))
      cat(sprintf("Covariates retained:  %s\n",
                  fmt_text_long(x$model$covariates_used)))
      ...
      cat(sprintf("Observations used: %s | logLik: %s | AIC: %s | BIC: %s\n",
                  ...
                  fmt_num_long(ll), fmt_num_long(aic), fmt_num_long(bic)))
  ...
  if (model && isTRUE(ov$arm_analysis)) {
    ...
    section_long("GLOBAL TIME-BY-ARM TESTS")
  ...
  # ------------------------------------------------------------------
  if (model && !isTRUE(ov$arm_analysis)) {
    section_long("GLOBAL TEST OF TIME")
    if (!is.null(x$model$global_time_test)) {
  ...
  if (model && (!is.null(x$advanced_tests) || !is.null(x$advanced_models))) {
    section_long("ADVANCED LONGITUDINAL INFERENCE")

    rm <- x$advanced_tests$rm_anova
    cat("RM-ANOVA:\n")
    if (!is.null(rm) && isTRUE(rm$performed) && !is.null(rm$tidy)) {
      for (question in c("TIME", "ARM", "TIME_X_ARM")) {
        idx <- .mira_match_effect_long(rm$tidy, question)
        if (length(idx) == 0L) next
        row <- rm$tidy[idx[[1L]], , drop = FALSE]
        gg <- if ("p_gg" %in% names(row)) fmt_p_long(row$p_gg) else "NA"
        hf <- if ("p_hf" %in% names(row)) fmt_p_long(row$p_hf) else "NA"
        cat(sprintf("  %s: F=%s | p=%s | GG p=%s | HF p=%s\n",
                    question, fmt_num_long(row$statistic), fmt_p_long(row$p_value), gg, hf))
      }
    ...
    if (!is.null(friedman) && isTRUE(friedman$performed)) {
      ...
      cat(sprintf("  statistic=%s | df=%s | p=%s | Kendall W=%s\n",
                  fmt_num_long(row$statistic), fmt_num_long(row$df, 0L),
                  fmt_p_long(row$p_raw), fmt_num_long(row$kendalls_w)))
    ...
    for (structure in c("independence", "exchangeable", "ar1")) {
      ...
      if (!is.null(item) && isTRUE(item$performed)) {
        ...
        gee_tests <- c(
          if (nrow(time_row) > 0L) paste0("TIME p=", fmt_p_long(time_row$p_raw[[1L]])),
          if (nrow(arm_row) > 0L) paste0("ARM p=", fmt_p_long(arm_row$p_raw[[1L]])),
          if (nrow(interaction_row) > 0L) {
            paste0("TIME-BY-ARM p=", fmt_p_long(interaction_row$p_raw[[1L]]))
          }
        )
    ...
    for (name in c("compound_symmetry", "ar1")) {
      ...
      if (!is.null(item) && isTRUE(item$performed)) {
        cat(sprintf("  %s: available | AIC=%s | BIC=%s\n",
                    label, fmt_num_long(item$AIC), fmt_num_long(item$BIC)))
      } else {
    ...
    if (has_rows_long(comparison) && "object_path" %in% names(comparison)) {
    ...
    if (!is.null(emmeans) && isTRUE(emmeans$performed)) {
      ...
      if (!is.null(emmeans_baseline_final) && nrow(emmeans_baseline_final) > 0L) {
        cat(sprintf("  baseline vs final: estimate=%s | primary-adjusted p=%s\n",
                    fmt_num_long(emmeans_baseline_final$estimate[[1L]]),
                    fmt_p_long(emmeans_baseline_final$p_primary[[1L]])))
      }
    ...
    if (is.data.frame(sensitivity) && nrow(sensitivity) > 0L) {
      ...
      range_text <- if (length(available_p) > 0L) {
        paste0(fmt_p_long(min(available_p)), " to ", fmt_p_long(max(available_p)))
      } else "NA"
      ...
      sensitivity_print <- data.frame(
        ...
        df = sensitivity$df,
        p = vapply(sensitivity$p_raw, fmt_p_long, character(1L)),
        Effect_size = round(sensitivity$effect_size, digits),
        Effect_size_type = sensitivity$effect_size_type,
        N = sensitivity$n,
        check.names = FALSE
      )
      print(limit_table_long(sensitivity_print, "sensitivity rows"), row.names = FALSE)
  ...
  add_r2_row_long <- function(label, item, object_path) {
    if (!model || is.null(item) || !isTRUE(item$performed) || !has_rows_long(item$tidy)) {
      return(NULL)
    }
    data.frame(
      Model = label,
  ...
  if (has_rows_long(rm_effects) || has_rows_long(rank_effects) || length(r2_rows) > 0L) {
    section_long("ADDITIONAL EFFECT SIZES")

    if (has_rows_long(rm_effects)) {
    ...
    if (has_rows_long(rank_effects)) {
      ...
      print(limit_table_long(rank_effects_print, "rank-biserial comparisons"), row.names = FALSE)
  ...
  family_rows <- Filter(Negate(is.null), lapply(family_entries, function(entry) {
    family <- entry[[2L]]
    if (is.null(family) || !has_rows_long(family$table)) return(NULL)
    data.frame(
      Family = entry[[1L]],
      Comparisons = nrow(family$table),
      Primary_adjustment = fmt_text_long(family$primary_method, "not recorded"),
      Table = entry[[3L]],
      stringsAsFactors = FALSE,
      check.names = FALSE
    )
  ...
  if (length(family_rows) > 0L) {
    section_long("MULTIPLICITY CONTROL")
    ...
    available_adjustments <- unique(unlist(lapply(family_entries, function(entry) {
      family <- entry[[2L]]
      if (is.null(family) || !has_rows_long(family$table)) character(0) else
  ...
  if (outliers) {
    ...
    if (is.null(x$outliers$by_time)) {
      ...
      if (!is.null(flagged_time) && nrow(flagged_time) > 0L) {
        ...
        flagged_time <- limit_table_long(flagged_time, "timepoint outlier flags")
      ...
      if (!is.null(x$outliers$change)) {
        ...
        if (!is.null(flagged_change) && nrow(flagged_change) > 0L) {
          ...
          flagged_change <- limit_table_long(flagged_change, "change outlier flags")
  ...
  if (trajectories && !is.null(x$trajectories)) {
    section_long("INDIVIDUAL TRAJECTORIES")
    tr <- x$trajectories
    finite_change <- tr$absolute_change[is.finite(tr$absolute_change)]

    cat(sprintf("Subjects with baseline-final data: %d/%d (%s)\n",
                length(finite_change), nrow(tr), fmt_pct_long(length(finite_change) / nrow(tr) * 100)))

    if (length(finite_change) > 0L) {
      cat(sprintf("Mean individual change: %s | SD: %s | Median: %s\n",
                  fmt_num_long(mean(finite_change)), fmt_num_long(stats::sd(finite_change)),
                  fmt_num_long(stats::median(finite_change))))
      cat(sprintf("Range: [%s, %s]\n", fmt_num_long(min(finite_change)), fmt_num_long(max(finite_change))))
    }
    ...
    tr_print <- limit_table_long(tr_print, "patient trajectories")
  ...
  is_adaptation_item_long <- function(item) {
    is.list(item) &&
      all(c("available_by_time", "variable_timepoints", "disabled") %in% names(item))
  }
  adaptation_items <- if (is_adaptation_item_long(adaptation)) {
    stats::setNames(
      list(adaptation),
      fmt_text_long(x$outcome_display, fmt_text_long(x$outcome, "Outcome"))
    )
  ...
  adaptation_lines <- unlist(lapply(seq_along(adaptation_items), function(i) {
    ...
    if (!is_adaptation_item_long(item)) return(character(0))
  ...
  if (length(diagnostic_warnings) > 0L || length(adaptation_lines) > 0L) {
    section_long("DIAGNOSTICS AND AUTOMATIC ADAPTATIONS")
  ...
  # ------------------------------------------------------------------
  .mira_print_output_guide_long(x, title = "COMPLETE OUTPUT GUIDE")
...
#' @export
summary.mira_info_long <- function(object, ...) {
  ov <- object$overview
  ch <- object$change
  ...
  class(out) <- c("summary.mira_info_long", "list")
  out
}

#' @method print summary.mira_info_long
#' @export
print.summary.mira_info_long <- function(x, digits = 3, ...) {
  cat("MIRA INFO\n")
  cat(strrep("=", 84L), "\n", sep = "")
  cat("\nSUMMARY\n")
  cat(strrep("-", 84L), "\n", sep = "")
...
plot.mira_info_long <- function(
  ...
  if (length(x$plots) == 0L || is.null(x$plots[[which]])) {
    stop(
      sprintf(
        "Plot '%s' is not available. Run mira_info_long(..., plots = TRUE) with ggplot2 installed.",
        which
      ),
      call. = FALSE
    )
...
#' @export
summary.mira_info_multi_long <- function(object, ...) {
  summaries <- lapply(object$outcomes, function(result) {
    if (inherits(result, "mira_info_error_long")) return(result)
    summary.mira_info_long(result, ...)
  })
  out <- list(
  ...
  class(out) <- c("summary.mira_info_multi_long", "list")
  out
}

#' @method print summary.mira_info_multi_long
#' @export
print.summary.mira_info_multi_long <- function(x, digits = 3, ...) {
  cat(sprintf("mira_info_long multi-outcome summary - %d outcomes\n", length(x$outcomes)))
  for (outcome in names(x$outcomes)) {
    cat("\n")
    value <- x$outcomes[[outcome]]
    if (inherits(value, "mira_info_error_long")) {
      print(value)
    } else {
      print.summary.mira_info_long(value, digits = digits, ...)
    }
  }
  invisible(x)
}
...
#' @export
plot.mira_info_multi_long <- function(x, outcome = NULL, which = "boxplot", ...) {
  ...
  idx <- which(.mira_key_long(available) == .mira_key_long(outcome))
  if (length(idx) != 1L) {
    stop(sprintf("Outcome '%s' is not available. Available values: %s.",
                 outcome, paste(available, collapse = ", ")), call. = FALSE)
  }
  plot.mira_info_long(x$outcomes[[available[[idx]]]], which = which, ...)
}
