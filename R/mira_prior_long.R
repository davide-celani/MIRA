...
.mira_normalize_outcome_long <- function(
    outcome,
    allow_auto = TRUE,
...
.mira_normalize_likelihood_prior_long <- function(likelihood, outcome) {
...
.mira_normalize_informativeness_long <- function(informativeness) {
...
.mira_apply_informativeness_long <- function(values, informativeness) {
...
.mira_normalize_custom_prior_long <- function(custom_prior) {
...
.mira_expand_covariate_prior_long <- function(value, field, P, covariate_names) {
...
.mira_apply_covariate_overrides_long <- function(values, overrides, covariate_map) {
...
#'
#' @param stan_data Data prepared by [mira_data_long()].
...
#'   stored by [mira_data_long()] (or Student-t for BCVA/generic and
...
#'
#' @return An object of class `mira_prior_long`.
#'
#' @export
mira_prior_long <- function(
    stan_data,
  ...
  if (!is.null(profile)) {
    ...
    if (legacy_profile %in% c("bcva", "cmt")) {
      ...
      if (!outcome_was_missing &&
          .mira_normalize_outcome_long(outcome[1L]) != legacy_outcome) {
    ...
    if (!is.null(legacy_information)) {
      if (!informativeness_was_missing &&
          .mira_normalize_informativeness_long(informativeness) !=
          legacy_information) {
  ...
  requested_outcome <- .mira_normalize_outcome_long(outcome[1L])
  informativeness <- .mira_normalize_informativeness_long(informativeness)
  time_unit <- match.arg(
    tolower(time_unit),
  ...
  data_likelihood <- if (!is.null(stan_data$likelihood)) {
    .mira_normalize_likelihood_prior_long(stan_data$likelihood, resolved_outcome)
  ...
  resolved_likelihood <- if (likelihood_requested == "auto" &&
    ...
    .mira_normalize_likelihood_prior_long(likelihood, resolved_outcome)
  }
  ...
  values <- .mira_apply_informativeness_long(values, informativeness)

  for (name in .mira_covariate_prior_user_fields) {
    values[[name]] <- .mira_expand_covariate_prior_long(
      values[[name]], name, P, covariate_names
    )
  }

  custom_specification <- .mira_normalize_custom_prior_long(custom_prior)
  ...
  named_custom <- .mira_apply_covariate_overrides_long(
    values,
...
#' @param ... Optional prior replacements passed to [mira_prior_long()].
#'
#' @return A `mira_prior_long` object.
#'
#' @export
mira_prior_bcva_long <- function(
    stan_data,
...
#'
#' @param prior A `mira_prior_long` object or complete named list.
#'
#' @return Invisibly returns `TRUE`.
#'
#' @export
mira_validate_prior_long <- function(prior) {
  if (!is.list(prior)) {
...
#'
#' @export
mira_prior_stan_data_long <- function(prior) {
  mira_validate_prior_long(prior)
...
#'
#' @param x A `mira_prior_long` object.
#' @param ... Additional arguments (unused).
#'
#' @export
print.mira_prior_long <- function(x, ...) {
  mira_validate_prior_long(x)
