...
report_test_mira <- function(outcome = "score") {
  ...
  structure(result, class = c("mira_info", "list"))
}
...
report_test_run <- function(..., .capture = TRUE) {
  if (!.capture) return(mira_report_freq(...))
  printed <- utils::capture.output(value <- mira_report_freq(...))
...
report_test_call <- function(args = list(), output_dir = tempfile("mira-report-test-")) {
  ...
  printed <- utils::capture.output(value <- do.call(mira_report_freq, call_args))
  attr(value, "test_printed_output") <- printed
  value
}
