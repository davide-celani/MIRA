...
test_that("A: NULL and character(0) produce a complete P = 0 pipeline", {
  ...
  for (stan_data in list(none, null)) {
    ...
    prior_data <- mira_prior_stan_data_long(prior)
...
test_that("A: covariate initial values are omitted for P = 0", {
  zero <- .mira_covariate_initial_values_long(P = 0L, K = 5L, rw_scale = 0.25)
  active <- .mira_covariate_initial_values_long(P = 2L, K = 5L, rw_scale = 0.25)
...
test_that("summary rejects same-P metadata from a different design matrix", {
  ...
  expect_error(
    mira_summary_long(fit, stan_data = gender_data, verbose = FALSE),
...
test_that("generic prior vectors follow active encoded columns and names", {
  ...
  direct <- mira_prior_long(many, covariate_baseline_sd = named_override)
  expect_equal(direct$covariate_baseline_prior_sd, named_override)

  nested <- mira_prior_long(
    many,
    custom_prior = list(
      covariates = list(
...
test_that("a prior cannot be reused for different encoded terms with the same P", {
  age_data <- prepare_dynamic_covariates("age")
  gender_data <- prepare_dynamic_covariates("gender")
  age_prior <- mira_prior_long(age_data)
  ...
  embedded <- age_data
  embedded_prior <- mira_prior_stan_data_long(age_prior)
