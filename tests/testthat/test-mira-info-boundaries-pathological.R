...
test_that("automatic arm detection includes its cardinality cutoff", {
  ...
  below <- MIRA:::.mira_detect_arm(make_data(10L))
  at <- MIRA:::.mira_detect_arm(make_data(11L))
  above <- MIRA:::.mira_detect_arm(make_data(12L))
...
test_that("automatic covariates include the degrees-of-freedom limit", {
  ...
  below <- MIRA:::.mira_detect_covariates(make_data(2L))
  at <- MIRA:::.mira_detect_covariates(make_data(3L))
  above <- MIRA:::.mira_detect_covariates(make_data(4L))
...
test_that("core summaries handle two rows, all missing values, and constants", {
  ...
  expect_identical(class(two), c("mira_info", "list"))
  ...
  expect_identical(class(all_missing), c("mira_info", "list"))
...
test_that("explicit configuration preserves non-syntactic column names", {
  ...
  expect_no_warning(result <- mira_info(
    data,
    id = "patient id",
  ...
  expect_identical(class(result), c("mira_info", "list"))
