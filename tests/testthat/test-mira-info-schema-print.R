...
test_that("single-outcome public schema preserves exact order, classes, and types", {
  ...
  expect_identical(class(x), c("mira_info", "list"))
...
test_that("inspect-only, multi-outcome, and failed-outcome schemas are exact", {
  ...
  expect_identical(class(inspect), c("mira_detect", "list"))
  ...
  expect_true(all(vapply(
    multi$outcomes,
    function(item) identical(class(item), c("mira_info", "list")),
    logical(1L)
  )))
  ...
  expect_identical(class(failure), c("mira_info_error", "list"))
  ...
  expect_identical(class(failed_summary), c("summary.mira_info_multi", "list"))
...
test_that("core public objects survive RDS serialization exactly", {
  ...
  expect_identical(class(restored$single), c("mira_info", "list"))
  expect_identical(class(restored$inspect), c("mira_detect", "list"))
  expect_identical(class(restored$multi), c("mira_info_multi", "mira_info", "list"))
})
...
test_that("multi, error, and summary print methods return invisibly", {
  ...
  x <- mira_schema_print_single_20260918()
  summary_x <- summary(x)
  expect_identical(class(summary_x), c("summary.mira_info", "list"))
  ...
  expect_identical(class(summary_multi), c("summary.mira_info_multi", "list"))
  ...
  expect_true(grepl(
    "mira_info multi-outcome summary - 2 outcomes",
