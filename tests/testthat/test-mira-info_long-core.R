...
test_that("core summaries remain numerically correct with every optional analysis off", {
  d <- core_mira_data()
  x <- mira_info_long(d, id = "patient_id", analyses = "none", verbose = FALSE)

  expect_s3_class(x, "mira_info_long")
...
test_that("response classification includes the exact stable threshold", {
  ...
  high <- mira_info_long(d, id = "patient_id", analyses = "none", verbose = FALSE,
                    improvement_direction = "higher", stable_threshold = 1)
  low <- mira_info_long(d, id = "patient_id", analyses = "none", verbose = FALSE,
...
test_that("pairwise p-value adjustment matches stats p.adjust within each family", {
  for (method in c("holm", "bonferroni", "BH", "BY", "none")) {
    x <- mira_info_long(core_mira_data(), id = "patient_id", analyses = "none",
...
test_that("correlations use pairwise complete samples and standard estimators", {
  ...
  x <- mira_info_long(d, id = "patient_id", analyses = "correlations", verbose = FALSE)
...
test_that("IQR flags identify a known extreme observation and change", {
  d <- data.frame(patient_id = seq_len(10), score_t0 = seq_len(10),
                  score_t1 = c(2:10, 100))
  x <- mira_info_long(d, id = "patient_id", analyses = "outliers", verbose = FALSE)
...
test_that("arm summaries use the chosen reference and estimate known differences", {
  ...
  x <- mira_info_long(d, id = "patient_id", arm = "arm", reference_arm = "control",
  ...
  off <- mira_info_long(d, id = "patient_id", arm = "arm", analyses = "none",
                   verbose = FALSE)
...
test_that("plot requests produce ggplot objects for the estimable core views", {
  ...
  d <- core_mira_data()
  x <- mira_info_long(d, id = "patient_id",
  ...
  off <- mira_info_long(d, id = "patient_id", analyses = "none", verbose = FALSE)
