# Direct tests for the configuration and data-preparation helpers used by mira_info_long().
...
test_that("longitudinal names recognize documented time suffixes and separators", {
  ...
  for (case in cases) {
    found <- MIRA:::.mira_parse_longitudinal_name_long(case[[1L]])
  ...
  expect_null(MIRA:::.mira_parse_longitudinal_name_long("age"))
  expect_null(MIRA:::.mira_parse_longitudinal_name_long("score_time"))
  expect_equal(MIRA:::.mira_time_order_long("BL"), 0)
  expect_true(is.na(MIRA:::.mira_time_order_long("followup")))
})

test_that("custom longitudinal regex and parser functions retain supplied metadata", {
  regex <- "^(.+)__at__(visit[0-9]+)$"
  hit <- MIRA:::.mira_parse_longitudinal_name_long("pain__at__visit12", regex)
  ...
  expect_null(MIRA:::.mira_parse_longitudinal_name_long("pain_t1", regex))
  expect_null(MIRA:::.mira_parse_longitudinal_name_long("pain_12", "^(.+)_([0-9]+)_x$"))

  parser <- function(x) {
    if (x == "ignore") return(NULL)
    list(outcome = "pain", time_label = "week6", time_order = 99)
  }
  hit <- MIRA:::.mira_parse_longitudinal_name_long("measurement", parser)
  expect_equal(hit$time_order, 99)
  expect_equal(hit$pattern, "custom_function")
  expect_null(MIRA:::.mira_parse_longitudinal_name_long("ignore", parser))

  dataframe_parser <- function(x) data.frame(outcome = "pain", time_label = "month3")
  hit <- MIRA:::.mira_parse_longitudinal_name_long("measurement", dataframe_parser)
  expect_equal(hit$time_order, 3)
  expect_equal(hit$pattern, "custom_function")
  expect_equal(MIRA:::.mira_parse_longitudinal_name_long("measurement",
    function(x) list(outcome = "pain"))$time_label, "measurement")

  expect_error(MIRA:::.mira_parse_longitudinal_name_long("x", function(x) "bad"),
               "list containing at least 'outcome'")
  expect_error(MIRA:::.mira_parse_longitudinal_name_long("x", function(x) list(time_label = "t1")),
               "list containing at least 'outcome'")
  expect_error(MIRA:::.mira_parse_longitudinal_name_long("x", 1), "variable_pattern")
  expect_error(MIRA:::.mira_parse_longitudinal_name_long("x", NA_character_), "variable_pattern")
})
...
test_that("explicit longitudinal groups sort clear time order and preserve ambiguous order", {
  ...
  ordered <- MIRA:::.mira_make_longitudinal_group_long(
  ...
  ambiguous <- MIRA:::.mira_make_longitudinal_group_long(
  ...
  expect_error(MIRA:::.mira_make_longitudinal_group_long(dat, "score_t0"), "at least two")
  expect_error(MIRA:::.mira_make_longitudinal_group_long(dat, c("score_t0", "score_t0")),
               "duplicate")
  expect_error(MIRA:::.mira_make_longitudinal_group_long(dat, c("score_t0", "missing")),
               "not found")
  dat$score_t3 <- letters[1:6]
  expect_error(MIRA:::.mira_make_longitudinal_group_long(dat, c("score_t0", "score_t3")),
               "must be numeric")
})
...
test_that("automatic longitudinal detection separates valid, ambiguous, and nonnumeric matches", {
  ...
  result <- MIRA:::.mira_detect_longitudinal_variables_long(dat)
  ...
  found <- MIRA:::.mira_detect_longitudinal_variables_long(duplicate_labels)
...
test_that("outcome resolution accepts explicit columns, grouped lists, and named labels", {
  ...
  detected <- MIRA:::.mira_detect_longitudinal_variables_long(dat)
  selected <- MIRA:::.mira_resolve_outcome_groups_long(
  ...
  by_columns <- MIRA:::.mira_resolve_outcome_groups_long(
  ...
  manual <- MIRA:::.mira_resolve_outcome_groups_long(
    dat, detected,
  ...
  expect_error(MIRA:::.mira_resolve_outcome_groups_long(dat, detected, outcomes = "absent"),
               "not detected")
  expect_error(MIRA:::.mira_resolve_outcome_groups_long(dat, detected,
...
test_that("ID detection requires a unique semantic candidate and records ambiguity", {
  ...
  chosen <- MIRA:::.mira_detect_id_long(dat, exclude = c("measurement_t0", "measurement_t1"))
  ...
  manual <- MIRA:::.mira_detect_id_long(dat, id = "patient_id")
  expect_false(manual$automatic)
  expect_equal(manual$source, "manual")
  expect_error(MIRA:::.mira_detect_id_long(dat, id = "absent"), "does not exist")

  ambiguous <- dat
  ambiguous$subject_id <- sprintf("S%02d", 1:12)
  tie <- MIRA:::.mira_detect_id_long(ambiguous,
  ...
  generated <- MIRA:::.mira_detect_id_long(no_semantic, exclude = c("y_t0", "y_t1"))
  ...
  detected <- MIRA:::.mira_detect_id_long(invalid,
    exclude = c("measurement_t0", "measurement_t1"))
...
test_that("arm detection checks names and cardinality while manual selection takes precedence", {
  ...
  found <- MIRA:::.mira_detect_arm_long(dat, exclude = c("score_t0", "score_t1"))
  ...
  rejected <- MIRA:::.mira_detect_arm_long(dat, exclude = c("score_t0", "score_t1"))
  expect_equal(rejected$selected, "treatment_arm")
  expect_match(rejected$warnings, "implausible cardinality")

  dat$arm <- "only"
  manual <- MIRA:::.mira_detect_arm_long(dat, arm = "arm")
  expect_equal(manual$selected, "arm")
  expect_false(manual$automatic)
  expect_error(MIRA:::.mira_detect_arm_long(dat, arm = "missing"), "does not exist")
})
...
test_that("reference arms prefer controls and otherwise the largest observed group", {
  ...
  expect_equal(MIRA:::.mira_choose_reference_arm_long(dat, "arm")$value, "control")
  expect_equal(MIRA:::.mira_choose_reference_arm_long(dat, "arm", "drug")$value, "drug")
  expect_false(MIRA:::.mira_choose_reference_arm_long(dat, "arm", "drug")$automatic)
  expect_error(MIRA:::.mira_choose_reference_arm_long(dat, "arm", "absent"), "not present")
  expect_error(MIRA:::.mira_choose_reference_arm_long(dat, NULL, "control"), "requires an arm")

  no_control <- data.frame(arm = c("B", "A", "B", "A", "B"))
  expect_equal(MIRA:::.mira_choose_reference_arm_long(no_control, "arm")$value, "B")
  multiple_controls <- data.frame(arm = c("control", "placebo", "placebo", "drug"))
  ref <- MIRA:::.mira_choose_reference_arm_long(multiple_controls, "arm")
...
test_that("covariate classification distinguishes continuous and categorical inputs", {
  dat <- data.frame(age = seq(20, 58, by = 2),
                    ...
                    constant = 1)
  classified <- MIRA:::.mira_classify_covariates_long(dat, names(dat))
...
test_that("covariate detection excludes identifiers and enforces manual validation", {
  ...
  found <- MIRA:::.mira_detect_covariates_long(
  ...
  manual <- MIRA:::.mira_detect_covariates_long(dat, c("age", "sex"),
    exclude = c("pain_t0", "pain_t1"))
  expect_false(manual$automatic)
  expect_equal(manual$numeric, "age")
  expect_equal(manual$categorical, "sex")
  expect_error(MIRA:::.mira_detect_covariates_long(dat, c("age", "age")), "duplicate")
  expect_error(MIRA:::.mira_detect_covariates_long(dat, "absent"), "not found")
  expect_error(MIRA:::.mira_detect_covariates_long(dat, "pain_t0",
    exclude = "pain_t0"), "already used")
  dat$patient <- seq_len(nrow(dat))
  expect_error(MIRA:::.mira_detect_covariates_long(dat, "patient"), "reserved")
  expect_error(MIRA:::.mira_detect_covariates_long(dat, 1), "covariates must")
})
...
test_that("direction and threshold resolution validate outcome-specific settings", {
  expect_equal(MIRA:::.mira_resolve_direction_long("auto", "BCVA")$value, "higher")
  expect_equal(MIRA:::.mira_resolve_direction_long("auto", "cmt")$value, "lower")
  expect_equal(MIRA:::.mira_resolve_direction_long("auto", "pain")$value, "unknown")
  expect_equal(MIRA:::.mira_resolve_direction_long("lower", "pain")$reason, "user")
  expect_equal(MIRA:::.mira_resolve_direction_long(
    c(Pain = "lower", BCVA = "higher"), "pain")$value, "lower")
  expect_error(MIRA:::.mira_resolve_direction_long("invalid", "pain"), "Allowed values")
  expect_error(MIRA:::.mira_resolve_direction_long(c("lower", "higher"), "pain"),
               "must be named")
  expect_error(MIRA:::.mira_resolve_direction_long(c(other = "lower"), "pain"),
               "does not contain exactly one")

  expect_equal(MIRA:::.mira_resolve_threshold_long("auto", "pain")$value, 0)
  expect_equal(MIRA:::.mira_resolve_threshold_long(NULL, "pain")$reason,
               "safe_zero_fallback")
  expect_equal(MIRA:::.mira_resolve_threshold_long(c(Pain = 1.5, BCVA = 2), "pain")$value,
               1.5)
  expect_error(MIRA:::.mira_resolve_threshold_long(-1, "pain"), "must be >= 0")
  expect_error(MIRA:::.mira_resolve_threshold_long(Inf, "pain"), "numeric value")
  expect_error(MIRA:::.mira_resolve_threshold_long(c(1, 2), "pain"), "must be named")
})
...
test_that("configuration builder records generated IDs, provenance, and column profiles", {
  dat <- data.frame(score_t0 = 1:12, score_t1 = 2:13,
                    ...
                    check.names = FALSE)
  built <- MIRA:::.mira_build_analysis_config_long(dat)
  ...
  manual <- MIRA:::.mira_build_analysis_config_long(dat, id = "patient_id",
...
test_that("configuration builder validates wide-format IDs and structural data", {
  ...
  expect_error(MIRA:::.mira_build_analysis_config_long(dat, id = "patient_id"),
               "duplicate values")
  permissive <- MIRA:::.mira_build_analysis_config_long(dat, id = "patient_id",
                                                     strict_id = FALSE)
  expect_match(permissive$warnings, "duplicates")
  dat$patient_id[[4L]] <- NA_character_
  expect_error(MIRA:::.mira_build_analysis_config_long(dat, id = "patient_id"),
               "missing values")
  expect_error(MIRA:::.mira_build_analysis_config_long(data.frame()),
               "no observations")
  duplicate_names <- dat
  names(duplicate_names)[[3L]] <- "score_t0"
  expect_error(MIRA:::.mira_build_analysis_config_long(duplicate_names),
               "duplicate column names")
  expect_error(MIRA:::.mira_build_analysis_config_long(dat, strict_id = NA),
               "strict_id")
})
...
test_that("blank categorical covariates are excluded from advanced model data", {
  ...
  prepared <- MIRA:::.mira_prepare_advanced_data_long(
    long, "Start", FALSE, character(0), "sex", "sex")
  expect_equal(nrow(prepared), 1L)

  long$sex <- factor(long$sex)
  prepared_factor <- MIRA:::.mira_prepare_advanced_data_long(
    long, "Start", FALSE, character(0), "sex", "sex")
