test_that("missing optional engines yield structured module skips", {
  ...
  modules <- testthat::with_mocked_bindings(
    code = list(
      emmeans = MIRA:::.mira_run_emmeans_long(
        dummy_model,
        arm_enabled = FALSE,
        primary_method = "holm",
        confidence_level = 0.95
      ),
      rm_anova = MIRA:::.mira_run_rm_anova_long(
        data.frame(),
        arm_enabled = FALSE,
        covariates = character(0),
        categorical_covariates = character(0)
      ),
      gee = MIRA:::.mira_run_gee_long(data.frame(), formula, 0.95),
      random_slope = MIRA:::.mira_run_random_slope_long(
        data.frame(),
        arm_enabled = FALSE,
        covariates = character(0),
        mixed_intercept_model = NULL
      ),
      nlme = MIRA:::.mira_run_nlme_long(data.frame(), formula),
      robust = MIRA:::.mira_run_robust_inference_long(dummy_model, 0.95),
      r2 = MIRA:::.mira_run_r2_long(dummy_model)
    ),
...
test_that("missing model and plotting packages do not destroy core public results", {
  ...
  expect_s3_class(result, "mira_info_long")
