...
test_that("public scalar arguments reject missing, wrong-type, and nonscalar values", {
  ...
  for (argument in c("arm_tests", "plots", "model", "outliers",
    ...
    for (value in list(NA, NULL, 1, 0, "TRUE", c(TRUE, FALSE), logical(0))) {
      ...
      expect_error(do.call(mira_info_long, args),
  ...
  expect_error(mira_info_long(matrix(1:4, 2), inspect_only = TRUE, verbose = FALSE),
               "data must be a data.frame")
  expect_error(mira_info_long(data[FALSE, ], inspect_only = TRUE, verbose = FALSE),
               "no observations")
  names(data)[2:3] <- "duplicate"
  expect_error(mira_info_long(data, inspect_only = TRUE, verbose = FALSE),
...
test_that("inspection reports resolved configuration and exits before analysis", {
  ...
  x <- visible$value
  expect_s3_class(x, "mira_detect_long")
  expect_identical(class(x), c("mira_detect_long", "list"))
...
test_that("manual and automatic ID rules are distinguished", {
  ...
  expect_warning(mira_info_long(duplicate, id = "record_code", strict_id = FALSE,
  ...
  expect_warning(mira_info_long(missing, id = "record_code", strict_id = FALSE,
                           inspect_only = TRUE, analyses = "none", verbose = FALSE,
                           covariates = character(0)),
                 "missing values")
...
test_that("single and multi-outcome public results have stable shape", {
  ...
  single <- mira_info_long(data, id = "subject_id", analyses = "none", verbose = FALSE,
                      covariates = character(0))
  expect_s3_class(single, "mira_info_long")
  ...
  multi <- mira_info_long(data, id = "subject_id", analyses = "none", verbose = FALSE,
                     covariates = character(0))
  ...
  expect_true(all(vapply(multi$outcomes, inherits, logical(1), "mira_info_long")))
  ...
  empty <- mira_info_long(data, id = "subject_id", outcomes = character(0),
                     analyses = "none", verbose = FALSE,
                     covariates = character(0))
  expect_s3_class(empty, "mira_info_multi_long")
...
test_that("requested modules record per-outcome data adaptations", {
  ...
  x <- mira_info_long(empty, id = "subject_id", covariates = character(0),
  ...
  y <- mira_info_long(mixed, id = "subject_id", covariates = character(0),
                 analyses = "correlations", verbose = FALSE)
  expect_s3_class(y, "mira_info_multi_long")
...
test_that("one outcome failure is isolated in a multi-outcome call", {
  ...
  data$bad_t1 <- 2:7
  expect_error(mira_info_long(data, id = "subject_id", outcomes = "bad",
                         covariates = character(0), analyses = "none",
                         verbose = FALSE))
  expect_warning(
    multi <- mira_info_long(data, id = "subject_id", covariates = character(0),
                       analyses = "none", verbose = FALSE),
    "Analyses were not completed for: bad"
  )
  expect_s3_class(multi, "mira_info_multi_long")
  expect_s3_class(multi$outcomes$good, "mira_info_long")
  expect_s3_class(multi$outcomes$bad, "mira_info_error_long")
