test_that("small scalar and presentation helpers handle empty and unusual values", {
  expect_identical(.mira_report_or(NULL, "fallback"), "fallback")
  expect_identical(.mira_report_or(character(), "fallback"), "fallback")
  expect_identical(.mira_report_or(FALSE, TRUE), FALSE)
  expect_identical(.mira_report_or(0, 1), 0)

  empty_table <- table(factor(character(), levels = character()))
  expect_true(.mira_report_is_empty(NULL))
  expect_true(.mira_report_is_empty(empty_table))
  expect_true(.mira_report_is_empty(data.frame()))
  expect_true(.mira_report_is_empty(matrix(numeric(), nrow = 0L)))
  expect_true(.mira_report_is_empty(list()))
  expect_false(.mira_report_is_empty(new.env()))
  expect_false(.mira_report_is_empty(data.frame(x = NA_real_)))

  heading <- capture.output(.mira_report_heading("A\nheading", 99L))
  expect_match(paste(heading, collapse = "\n"), "###### A heading", fixed = TRUE)
  expect_identical(.mira_report_human_name("data_overview"), "Dataset overview")
  expect_identical(.mira_report_human_name("new.result_name"), "New result name")
  expect_identical(.mira_report_human_name("___"), "Unnamed item")

  expect_identical(.mira_report_inline(NULL), "not available")
  expect_identical(.mira_report_inline(as.Date(c("2025-01-01", "2025-01-02"))),
                   "2025-01-01, 2025-01-02")
  expect_identical(.mira_report_inline(c(a = 1, b = NA)), "1, NA")
  expect_match(.mira_report_inline(list(alpha = 1)), "alpha")
})
...
test_that("p-value formatting is exact at the reporting threshold", {
  values <- c(NA, NaN, Inf, -Inf, 0, 0.0001, 0.0009, 0.001,
              0.0011, 0.05, 1, -0.1)
  expect_identical(
    .mira_report_format_p(values, digits = 3L),
  ...
  for (digits in c(1L, 2L, 4L, 10L)) {
    threshold <- 10^(-digits)
    formatted <- .mira_report_format_p(
  ...
  expect_identical(.mira_report_format_number(NULL), "not estimable")
  expect_identical(.mira_report_format_number(NA_real_), "not estimable")
  expect_identical(.mira_report_format_number(1.23456, 3L), "1.23")
  expect_identical(.mira_report_format_number(Inf), "Inf")
  expect_identical(.mira_report_format_number(-Inf), "-Inf")
  expect_true(is.na(.mira_report_format_number("not numeric")))
})
...
test_that("prepare_table normalizes core tabular input classes", {
  numeric_frame <- data.frame(estimate = c(1.2345, NA), p.value = c(0.0004, 0.05),
                              check.names = FALSE)
  prepared <- .mira_report_prepare_table(numeric_frame, digits = 3L)
  ...
  mixed_out <- .mira_report_prepare_table(mixed)
  ...
  matrix_out <- .mira_report_prepare_table(matrix_value)
  ...
  table_out <- .mira_report_prepare_table(tab)
...
test_that("prepare_table handles vectors, lists, and complex data-frame columns", {
  ...
  complex_out <- .mira_report_prepare_table(complex)
...
test_that("p-column detection does not reformat unrelated numeric columns", {
  ...
  output <- .mira_report_prepare_table(input, digits = 3L)
...
test_that("numeric-column recognition uses values and complete name tokens", {
  ...
  for (name in recognized) {
    expect_true(
      .mira_report_numeric_column(c("not", "numeric"), name),
      info = name
    )
  }
  ...
  for (name in false_positives) {
    expect_false(
      .mira_report_numeric_column(c("low", "high"), name),
      info = name
    )
  }

  expect_true(.mira_report_numeric_column(c(rep("1", 9), "x"), "label"))
  expect_false(.mira_report_numeric_column(c(rep("1", 8), "x", "y"), "label"))
  expect_true(.mira_report_numeric_column(c("<0.001", "0.2", "--"), "label"))
  expect_false(.mira_report_numeric_column(c(NA_character_, "", "--"), "label"))
  expect_true(.mira_report_numeric_column(factor(c("1", "2")), "label"))
})

test_that("column widths and partitions preserve every non-key column in order", {
  expect_identical(.mira_report_column_widths(data.frame()), numeric())
  expect_identical(.mira_report_partition_columns(data.frame(), 4L),
                   list(integer()))

  small <- data.frame(id = 1:3, label = c("short", "a much longer value", "mid"),
                      estimate = c(1.1, 2.2, 3.3), stringsAsFactors = FALSE)
  widths <- .mira_report_column_widths(small)
  expect_length(widths, ncol(small))
  expect_true(all(is.finite(widths) & widths >= 4))
  expect_lte(widths[[1L]], 14)
  expect_lte(widths[[2L]], 34)
  expect_identical(.mira_report_partition_columns(small, 4L), list(1:3))
  ...
  blocks <- .mira_report_partition_columns(large, max_columns = 4L,
                                            width_budget = 22)
  ...
  unlimited <- .mira_report_partition_columns(large, max_columns = ncol(large),
                                                width_budget = Inf)
...
test_that("LaTeX helpers create valid alignment and repeat headers idempotently", {
  ...
  alignment <- .mira_report_latex_alignment(data)
  ...
  broken <- .mira_report_latex_breaks(escaped)
  ...
  expect_identical(.mira_report_latex_breaks("plain text"), "plain text")
  ...
  repeated <- .mira_report_repeat_longtable_header(longtable)
  expect_match(repeated, "\\endfirsthead", fixed = TRUE)
  expect_match(repeated, "\\endhead", fixed = TRUE)
  expect_identical(.mira_report_repeat_longtable_header(repeated), repeated)
  expect_identical(.mira_report_repeat_longtable_header("minimal"), "minimal")
  no_midrule <- "\\toprule\nheader"
  expect_identical(.mira_report_repeat_longtable_header(no_midrule), no_midrule)
  empty_header <- "\\toprule\n\\midrule"
  expect_identical(.mira_report_repeat_longtable_header(empty_header), empty_header)
})
...
test_that("table rendering supports pipe output, truncation, and fallback", {
  ...
  output <- capture.output(value <- .mira_report_table(data, "Results", cfg))
  ...
  fractional <- capture.output(
    fractional_value <- .mira_report_table(
  ...
  fallback <- capture.output(
    returned <- testthat::with_mocked_bindings(
      .mira_report_table(data[1, ], cfg = cfg),
...
test_that("HTML and LaTeX table branches add their format-specific safeguards", {
  ...
  html <- testthat::with_mocked_bindings(
    capture.output(.mira_report_table(data, "Model", cfg)),
    is_latex_output = function() FALSE,
...
test_that("CSV normalization preserves raw scientific values and atomic columns", {
  ...
  csv <- .mira_report_csv_data(frame)
  ...
  matrix_csv <- .mira_report_csv_data(matrix_value)
  ...
  expect_equal(sum(.mira_report_csv_data(tab)$Freq), 3)
  zero <- .mira_report_csv_data(data.frame(a = numeric(), p = numeric()))
  expect_identical(dim(zero), c(0L, 2L))
  expect_true(report_test_atomic_table(zero))
  expect_error(.mira_report_csv_data(new.env()))
})

test_that("filename and quoting helpers are portable and reversible", {
  expect_identical(.mira_report_file_slug("My analysis"), "my_analysis")
  expect_identical(.mira_report_file_slug("a/b:c"), "a_b_c")
  expect_identical(.mira_report_file_slug("résumé"), "resume")
  expect_identical(.mira_report_file_slug("___"), "item")
  expect_identical(.mira_report_file_slug(""), "item")
  expect_identical(.mira_report_slug("___"), "mira_report")

  reserved <- c("CON", "PRN", "AUX", "NUL", "COM1", "LPT9")
  expect_identical(
    unname(vapply(reserved, .mira_report_file_slug, character(1L))),
    paste0("item_", tolower(reserved))
  )
  expect_identical(
    unname(vapply(reserved, .mira_report_slug, character(1L))),
    paste0("mira_", tolower(reserved))
  )
  expect_lte(nchar(.mira_report_file_slug(strrep("a", 200L))), 90L)
  expect_lte(nchar(.mira_report_slug(strrep("a", 200L))), 90L)

  greek <- .mira_report_file_slug("α β γ")
  expect_true(nzchar(greek))
  expect_match(greek, "^[a-z0-9_]+$")

  expect_identical(.mira_report_yaml_quote("O'Brien\nanalysis"),
                   "'O''Brien analysis'")
  r_value <- "C:\\analysis\\a\"b"
  expect_identical(eval(parse(text = .mira_report_r_quote(r_value))), r_value)

  format_lines <- .mira_report_format_lines(c("html", "pdf", "docx"))
...
test_that("plot and model recognition covers base and optional display classes", {
  ...
  expect_true(.mira_report_is_model(fit))
  expect_true(.mira_report_is_model(test))
  expect_true(.mira_report_is_model(structure(
    list(), class = c("lmerModLmerTest", "lmerMod", "merMod")
  )))
  expect_false(.mira_report_is_model(data.frame(x = 1)))
  expect_true(.mira_report_is_plain_list(list(x = 1)))
  expect_false(.mira_report_is_plain_list(data.frame(x = 1)))
  expect_false(.mira_report_is_plain_list(fit))

  expect_true(.mira_report_is_plot(structure(list(), class = "recordedplot")))
  expect_true(.mira_report_is_plot(grid::rectGrob()))
  expect_false(.mira_report_is_plot(list(x = 1)))

  skip_if_not_installed("ggplot2")
  plot <- ggplot2::ggplot(mtcars, ggplot2::aes(wt, mpg)) + ggplot2::geom_point()
  expect_true(.mira_report_is_plot(plot))
  device <- withr::local_tempfile(fileext = ".pdf")
  grDevices::pdf(device)
  on.exit(grDevices::dev.off(), add = TRUE)
  output <- capture.output(printed <- .mira_report_plot(plot, "mean_ci"))
  expect_true(printed)
  ...
  output <- capture.output(
    printed <- testthat::with_mocked_bindings(
      .mira_report_plot(broken, "broken"),
