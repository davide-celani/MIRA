test_that("section routing filters outcome and context trees", {
  ...
  expect_identical(vapply(names(routes), .mira_report_section, character(1L)), routes)
  expect_identical(.mira_report_section("frequencies"), "frequencies")
  expect_true(.mira_report_selected(report_test_cfg("all"), "models"))
  expect_true(.mira_report_selected(report_test_cfg(c("overview", "models")), "models"))
  expect_false(.mira_report_selected(report_test_cfg("overview"), "models"))
  ...
  nodes <- .mira_report_outcome_tree(outcome, report_test_cfg(c("overview", "models")))
  ...
  expect_named(
    .mira_report_outcome_tree(frequency_outcome, report_test_cfg("frequencies")),
    "frequencies"
  )

  detected <- report_test_detect()
  expect_named(.mira_report_context_tree(detected, report_test_cfg("methods")), "config")
  expect_setequal(
    names(.mira_report_context_tree(detected, report_test_cfg("data_quality"))),
...
test_that("pruning classifies reportable leaves and rejects machinery", {
  ...
  for (name in names(tabular)) {
    node <- .mira_report_prune(tabular[[name]], name, paste0("outcome$", name), cfg)
  ...
  expect_identical(
    .mira_report_prune(plot, "plot", "outcome$plots$plot", cfg)$kind, "plot"
  )
  fitted <- stats::lm(y ~ x, data = data.frame(x = 1:3, y = c(1, 3, 6)))
  expect_identical(
    .mira_report_prune(fitted, "fit", "outcome$model$fit", cfg)$kind, "model"
  )
  expect_identical(
    .mira_report_prune(y ~ x, "formula", "outcome$formula", cfg)$kind, "formula"
  )
  expect_identical(
    .mira_report_prune(quote(mean(x)), "call_expression", "outcome$call_expression", cfg)$kind,
    "formula"
  )
  expect_identical(
    .mira_report_prune(expression(x + 1), "expression", "outcome$expression", cfg)$kind,
    "formula"
  )
  expect_identical(
    .mira_report_prune(as.name("x"), "symbol", "outcome$symbol", cfg)$kind, "formula"
  )
  expect_identical(
    .mira_report_prune(c(alpha = 1), "estimate", "outcome$estimate", cfg)$kind, "value"
  )
  ...
  rejected <- list(
    ...
    outcome_error = structure(
      list(error = "controlled"), class = c("mira_info_error", "list")
    ),
    all_missing = c(NA_real_, NA_real_)
  )
  for (name in names(rejected)) {
    expect_null(
      .mira_report_prune(rejected[[name]], name, paste0("outcome$", name), cfg),
      info = name
    )
  }
  ...
  for (key in hidden_keys) {
    expect_null(
      .mira_report_prune("internal", key, paste0("outcome$", key), cfg), info = key
    )
  }
  expect_null(.mira_report_prune(
  ...
  table_node <- .mira_report_prune(data.frame(x = 1), "table", "outcome$table", cfg)
  value_node <- .mira_report_prune(1, "value", "outcome$value", cfg)
  expect_true(.mira_report_node_has_result(table_node))
  expect_false(.mira_report_node_has_result(value_node))
  expect_true(.mira_report_node_has_result(list(
...
test_that("equivalent aliases and duplicate model objects appear only once", {
  ...
  node <- .mira_report_prune(module, "module", "outcome$advanced_tests$module", cfg)
  ...
  distinct_node <- .mira_report_prune(
...
test_that("NLME keeps only genuinely fitted correlation structures", {
  ...
  node <- .mira_report_prune(nlme, "nlme", "outcome$advanced_models$nlme", cfg)
...
test_that("all-missing correlations are omitted without hiding estimable matrices", {
  ...
  node <- .mira_report_outcome_tree(outcome, report_test_cfg("correlations"))$correlations
  ...
  expect_null(
    .mira_report_outcome_tree(outcome, report_test_cfg("correlations"))$correlations
  )
})
...
test_that("outlier trees distinguish insufficient samples from zero outliers", {
  ...
  node <- .mira_report_outcome_tree(outcome, report_test_cfg("outliers"))$outliers
  ...
  rendered_empty <- capture.output(.mira_report_render_node(
...
test_that("an empty mira_info_multi has no synthetic outcome", {
  empty_multi <- structure(
    ...
    class = c("mira_info_multi", "mira_info", "list")
  )
  expect_identical(.mira_report_extract_outcomes(empty_multi), list())

  context <- .mira_report_context_tree(empty_multi, report_test_cfg())
  expect_true(all(c("config", "data_overview", "diagnostics") %in% names(context)))
  archived <- .mira_report_collect_tables(empty_multi, report_test_cfg())
  paths <- vapply(archived, `[[`, character(1L), "path")
  expect_false(any(startsWith(paths, "outcome$")))

  rendered <- capture.output(.mira_report_knit(list(
    result = empty_multi, extra_objects = list(), report = report_test_cfg()
  )))
  expect_false(any(grepl("^# Outcome:", rendered)))
})

test_that("mira_info_error outcomes are surfaced only in diagnostics", {
  multi <- report_test_multi(include_error = TRUE)
  outcomes <- .mira_report_extract_outcomes(multi)
  expect_named(outcomes, c("score", "fatigue", "failed"))
  expect_identical(.mira_report_outcome_tree(outcomes$failed, report_test_cfg()), list())
  expect_null(.mira_report_prune(
    outcomes$failed, "failed", "result$outcomes$failed", report_test_cfg()
  ))

  context <- .mira_report_context_tree(multi, report_test_cfg("diagnostics"))
  ...
  without_diagnostics <- .mira_report_context_tree(multi, report_test_cfg("overview"))
