test_that("the report omits model sections without fitted results", {
  ...
  node <- .mira_report_prune(gee, "independence",
  ...
  outcome <- structure(list(model = primary), class = c("mira_info", "list"))
  nodes <- .mira_report_outcome_tree(outcome, cfg)
...
test_that("model comparison omits unavailable fits", {
  ...
  node <- .mira_report_prune(comparison, "model_comparison",
  ...
  expect_null(.mira_report_prune(comparison, "model_comparison",
                                 "outcome$advanced_models$model_comparison", cfg))
})

test_that("the report shows the analyses selected in mira_info configuration", {
  cfg <- list(sections = "all", max_depth = 50L)
  detected <- structure(
    list(config = list(analyses = list(core = TRUE, model = FALSE,
                                         correlations = TRUE))),
    class = c("mira_detect", "list")
  )
  context <- .mira_report_context_tree(detected, cfg)
...
test_that("GEE summaries are printed as summaries rather than internal lists", {
  ...
  node <- .mira_report_prune(summary, "summary",
                             "outcome$advanced_models$gee$ar1$summary", cfg)
  expect_identical(node$kind, "model")
  expect_identical(node$value, summary)
})

test_that("a report built from mira_info preserves the result and exports its tables", {
  data <- data.frame(
    patient = 1:6,
  ...
  nodes <- .mira_report_outcome_tree(fit, report$report_config)
  expect_null(nodes$model)
