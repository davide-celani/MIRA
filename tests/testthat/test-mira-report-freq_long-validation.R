test_that("mira_report_freq_long is part of the public package API", {
  expect_true("mira_report_freq_long" %in% getNamespaceExports("MIRA"))
  expect_s3_class(report_test_mira(), "mira_info_long")
})
...
test_that("input dispatch accepts documented classes and rejects conflicts", {
  ...
  expect_error(
    ...
    "Supply x (the output of mira_info_long) or data",
    fixed = TRUE
  )
  expect_error(
    report_test_call(list(x = structure(list(), class = "mira_info_error_long"))),
  ...
  expect_error(
    do.call(mira_report_freq_long, duplicate),
...
test_that("data dispatch forwards dots and supplies verbose FALSE by default", {
  ...
  testthat::local_mocked_bindings(mira_info_long = fake_mira_info, .package = "MIRA")
...
test_that("the missing mira_info_long dependency fails without changing global bindings", {
  expect_true(exists("mira_info_long", mode = "function", inherits = TRUE))
  isolated <- new.env(parent = baseenv())
  isolated$.mira_report_validate_scalar_flag_long <- .mira_report_validate_scalar_flag_long
  isolated_function <- mira_report_freq_long
  ...
  expect_error(
    ...
    "mira_info_long() is not available",
    fixed = TRUE
  )
  expect_true(exists("mira_info_long", mode = "function", inherits = TRUE))
})
